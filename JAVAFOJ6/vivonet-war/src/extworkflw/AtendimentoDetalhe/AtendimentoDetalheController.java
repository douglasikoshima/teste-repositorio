...
public class AtendimentoDetalheController extends AbstractAction {
	...
	protected ActionForward begin(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (inDisponivel != null && inDisponivel != ConstantesCRM.SVAZIO) {
	...
	protected ActionForward RDArvoreAtendimento(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		palitagem = palitagem.startsWith("/") ? palitagem.replaceFirst("/", "").trim() : palitagem;
	...
	protected ActionForward RDLinhasAssoc(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ListaDadosVO listaDadosVO = atendimentoFacade.obtemLinhasAssociadas(user, idAtendimento, "");
	...
	protected ActionForward RDLinhasAssocEx(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ListaDadosVO listaDadosVO = atendimentoFacade.obtemLinhasAssociadasEx(user, idAtendimento, "");
		rWFLinhasAssocForm.setLinhasAssoc(listaDadosVO);
	...
	protected ActionForward setGestorGerenteConta(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		log.debug(headLog + "NrDocumentoGestor = " + form.getNrCPF().replaceAll("[^0-9]", ""));
		param.setNrTelefoneCelular(form.getNrTelefone().replaceAll("[^0-9]", ""));
		log.debug(headLog + "NrTelefoneCelular = " + form.getNrTelefone().replaceAll("[^0-9]", ""));
		param.setNrTelefoneContato(form.getNrTelefoneContato().replaceAll("[^0-9]", ""));
		log.debug(headLog + "NrTelefoneContato = " + form.getNrTelefoneContato().replaceAll("[^0-9]", ""));
	 ...
	 */
	/*
	 * private void inicializaCamposFormatados() { String contaFormatada = ""; String linhaFormatada = ""; ContaVO[]
	...
	/*
	 * private void inicializaScriptPalitagem() { String script = ""; String palitagem =
	 ...
	 * palitagem.replaceFirst("/", "").trim() : palitagem; if(palitagem != null) {
	 */
	// String[] itens = palitagem.split("\\s*/\\s*");
	/*
	 ...
	 * .getAtendimentoDetalheVO().getAtendimentoVO().getEncerramentoVO() == null) return; String script = ""; String
	 * palitagem = this.atendimentoDetalheForm .getAtendimentoDetalheVO().getAtendimentoVO().getEncerramentoVO
	 * ().getDsBaixa().trim(); palitagem = palitagem.startsWith("/") ? palitagem.replaceFirst("/", "").trim() :
