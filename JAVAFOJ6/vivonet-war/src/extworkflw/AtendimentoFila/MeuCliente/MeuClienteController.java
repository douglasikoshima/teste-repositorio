package extworkflw.AtendimentoFila.MeuCliente;
...
import extworkflw.AtendimentoFila.MeuCliente.formBeans.FilaMeuClienteFiltrosForm;
...
public class MeuClienteController extends AbstractAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		if ("index".equals(mapping.getParameter())) {
	...
	protected ActionForward index(ActionMapping mapping, ActionForm formParam,
		...
		ConstantesCRM.setCurrentUser(request.getSession(),
				dadosTokenVivo360.getIdUsuarioAlteracao());
		request.getSession().setAttribute("dadosTokenVivo360",
				dadosTokenVivo360);
	...
	protected ActionForward getUsuariosByIdGrupo(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		//FilaMeuClienteFiltrosForm form = (FilaMeuClienteFiltrosForm) formParam;
		Log.debug("getUsuariosByIdGrupo inicio");
	...
	protected ActionForward pesquisarProcessos(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (resultadoPesquisa != null && resultadoPesquisa.getAtendimentoFilaVOArray() != null) {
				logger.debug("vamos alterar o caminho do detalhe do processo de /workflow para /extworkflw");
				int size = resultadoPesquisa.getAtendimentoFilaVOArray().length;
				for(int i=0; i < size; i++) {
					AtendimentoVO atd = resultadoPesquisa.getAtendimentoFilaVOArray(i).getAtendimentoVO();
					if (atd != null) {
						String url = atd.getNmURLDados();
						if (url != null) {
							url = url.replaceFirst("/FrontOfficeWeb/workflow/", "/FrontOfficeWeb/extworkflw/");
							logger.debug(url);
						}
						atd.setNmURLDados(url);
					}
				}
				logger.debug("alterado o caminho de todos os resultados de processo de /workflow para /extworkflw");
			}
	...
	protected ActionForward encaminharSuspeito(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		atendimentoForm.setFila("/FrontOfficeWeb/extworkflw/AtendimentoFila/MeuCliente/begin.do");
