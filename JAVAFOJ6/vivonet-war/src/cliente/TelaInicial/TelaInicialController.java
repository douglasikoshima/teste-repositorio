...
public class TelaInicialController extends AbstractAction {
	...
	private static final String SCLIENTES = "clientes";
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		} else if ("carregaRelacionamento".equals(mapping.getParameter())) {
			return carregaRelacionamento(mapping, form, request, response);
	...
	public ActionForward begin(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if ((request.getParameter("pesquisa") != null) && (!(ConstantesCRM.SVAZIO.equals(request.getParameter("pesquisa"))))) {
		...
		if ((request.getSession().getAttribute(ConstantesCRM.USUARIO_LOGIN_CTI) != null) && (!ConstantesCRM.SVAZIO.equals(request.getSession().getAttribute(ConstantesCRM.USUARIO_LOGIN_CTI)))) {
			setCampanhas();
		}
	...
	/*
	 * Action's do relacionamento
	 */
	/**
	 * @jpf:action
	 * @jpf:forward name="success" path="Relacionamento.jsp"
	 */
	public ActionForward carregaRelacionamento(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		
	    RelacionamentoForm form = (RelacionamentoForm) formParam;
		user = ConstantesCRM.getCurrentUser(request.getSession());
		getRelacionamentoForm().setHistoricoRelacionamento(false);
		logTelaInicial.debug("TelaInicialController:carregaRelacionamento(" + user + ") >> INICIALIZADO");
		...
		if (parametros != null) {
			...
			relacionamentoForm.setIdPessoaDePara(parametros.getIdPessoaCliente() == null ? "21" : parametros.getIdPessoaCliente());
			...
			relacionamentoForm.setIdLinha((parametros.getIdLinha() == null) || (ConstantesCRM.SVAZIO.equals(parametros.getIdLinha())) ? ConstantesCRM.SONE : parametros.getIdLinha());
		} else {
			relacionamentoForm.setIdPessoaDePara("21");
			relacionamentoForm.setTpRelacionamento(ConstantesCRM.SONE);
			relacionamentoForm.setIdLinha(ConstantesCRM.SONE);
		}
		
		relacionamentoForm.setWfEstados(relacionamentoFacade.obterEstados(user).getWFEstados());
		relacionamentoForm.setInPrimeiraChamada(ConstantesCRM.SVAZIO);
		
		if (parametros.getIdTipoRelacionamento() == null || ConstantesCRM.SVAZIO.equals(parametros.getIdTipoRelacionamento()) || ConstantesCRM.SZERO.equals(parametros.getIdTipoRelacionamento())) {
			...
			} else if (ConstantesCRM.SSIX.equals(parametros.getIdTipoRelacionamento())) {
				relacionamentoForm.setTipoPesquisa(0);
				relacionamentoForm.setTpPesquisa(ConstantesCRM.STHREE);
			}
		}
		
		if (relacionamentoForm.getTpPesquisa() != null && form.getInPrimeiraChamada() != null) {
			request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
			return pesquisarRelacionamento(mapping, relacionamentoForm, request, response);
		...
		logTelaInicial.debug("TelaInicialController:carregaRelacionamento(" + user + ") >> FINALIZADO");
		request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
		return mapping.findForward(ConstantesCRM.SUCCESS);
	}
	...
	public ActionForward getPesquisaHistorico(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (parametros != null) {
			...
			form.setIdLinha((parametros.getIdLinha() == null) || (ConstantesCRM.SVAZIO.equals(parametros.getIdLinha())) ? ConstantesCRM.SONE : parametros.getIdLinha());
		}
		...
		if (parametros != null) {
			...
			if (form.getTpPesquisa() != null) {
				...
				if (ConstantesCRM.SONE.equals(relacionamentoForm.getTpPesquisa())) {
	...
	public ActionForward obterArvoreContato(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for (int i = 0; i < admContatoFolhaVO.getAdmContatoFolhaVOArray().length; i++) {
			...
			if (!ConstantesCRM.SONE.equals(admContatoFolhaVO.getAdmContatoFolhaVOArray(i).getInFolha())) {
	 ...
	 */
	public ActionForward pesquisarRelacionamento(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		RelacionamentoForm form = (RelacionamentoForm) formParam;
		...
		// Verificar com inPrimeiraChamada
		atPesquisaVOgravar.setInPrimeiraChamada(form.getInPrimeiraChamada());
		atPesquisaVOgravar.setDtAberturaInicio(form.getDtAberturaInicio());
		atPesquisaVOgravar.setDtAberturaFim(form.getDtAberturaFim());
		atPesquisaVOgravar.setDtFechamentoInicio(form.getDtFechamentoInicio());
		...
		if (getRelacionamentoForm().isHistoricoRelacionamento()) {
			getRelacionamentoForm().setHistoricoRelacionamento(true);
			RelacionamentoDAO dao = new RelacionamentoDAO();
			AtendimentoRelacionamentosVO atendimento = dao.getRelacionamentoExpurgo(user, atPesquisaVOgravar);

			if (atendimento != null) {
				atendimento = dao.getAtendimentoPorProtocolo(user, atendimento, atPesquisaVOgravar);
			}
			relacionamentoForm.setAtendimentoRelacionamentosVO(atendimento);

		} else {
			if (getRelacionamentoForm().isHistoricoRelacionamentoMG()) {
				atPesquisaVOgravar.setCdOperacao("historicoMG");
			}
			...
			if (relacionamentoForm.getAtendimentoRelacionamentosVO() != null) {
				for (int cont = 0; cont < relacionamentoForm.getAtendimentoRelacionamentosVO().getAtendimentoRelacionamentoVOArray().length; cont++) {
					if (relacionamentoForm.getAtendimentoRelacionamentosVO().getAtendimentoRelacionamentoVOArray(cont).getAtendimentoVO().getNrTelefone() == null || ConstantesCRM.SVAZIO.equals(relacionamentoForm.getAtendimentoRelacionamentosVO().getAtendimentoRelacionamentoVOArray(cont).getAtendimentoVO().getNrTelefone())) {
					...
					if ("S".equals(relacionamentoForm.getAtendimentoRelacionamentosVO().getAtendimentoRelacionamentoVOArray(cont).getAtendimentoVO().getInACS())) {
						relacionamentoForm.setFlagACS(true);
					
					} else if ("N".equals(relacionamentoForm.getAtendimentoRelacionamentosVO().getAtendimentoRelacionamentoVOArray(cont).getAtendimentoVO().getInACS())) {
	...
	public ActionForward carregaParametrosCTI(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// fazer lógica para saber se está com senha validada se "S" marcar como true
			inSenhaValidada = request.getParameter("inSenhaValidada");
			this.validarClienteForm.setSenhaValidada("true".equals(inSenhaValidada));
			...
			this.validarClienteForm.setUsuario(ConstantesCRM.SONE.equals(idTipoRelacionamento));
			this.validarClienteForm.setCliente(ConstantesCRM.STWO.equals(idTipoRelacionamento));
	...
	public ActionForward carregarLigacao(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		formAux.setCliente(request.getParameter("idTipoRelacionamento") != null ? ConstantesCRM.STWO.equals(request.getParameter("idTipoRelacionamento")) : false);
		formAux.setUsuario(request.getParameter("idTipoRelacionamento") != null ? ConstantesCRM.SONE.equals(request.getParameter("idTipoRelacionamento")) : false);
	...
	public ActionForward TelaInicial(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			setCampanhas();
			if ((idTipoRelProspect != null && idTipoRelProspect.equals("6")) || nrLinha == null) {// PROSPECT
	 ...
	 * @jpf:forward name="linhasConvergentes" path="listaLinhasConvergentes.jsp"
	...
	public ActionForward pesquisaCliente(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (request.getParameter("protocoloAtivo") == null || "false".equals(request.getParameter("protocoloAtivo"))) {
			request.getSession().removeAttribute(ConstantesCRM.NRPROTOCOLO);
			getTiForm().setNrProtocolo(ConstantesCRM.SVAZIO);
			}
			...
			if (SCLIENTES.equals(request.getParameter("tipoLista")) && "protocolo".equals(pesquisa)) {
				try {
					...
					if (offset > 0) {
						pageNumber = (request.getParameter("pageNumber") != null ? String.valueOf(Integer.parseInt(request.getParameter("pageNumber")) + 1) : ConstantesCRM.SONE);
					} else {
						pageNumber = request.getParameter("pageNumber") != null ? String.valueOf(Integer.parseInt(request.getParameter("pageNumber")) - 1) : ConstantesCRM.SONE;
					}
			...
			} else if (SCLIENTES.equalsIgnoreCase(request.getParameter("tipoLista")) && !"conta".equals(pesquisa)) {
				logTelaInicial.debug(msgLogDebug + " pesquisa: " + pesquisa);
				valor = ConstantesCRM.SVAZIO;

				if (!"nome".equalsIgnoreCase(pesquisa) || !"razao".equals(pesquisa)) {
				...
				if (("celular".equalsIgnoreCase(pesquisa)) && (listaClientes.getClientesPesquisadosArray().length > 0)) {
					request.setAttribute("nrLinha", valor);
				...
				} else if (("celular".equalsIgnoreCase(pesquisa)) && (listaClientes.getClientesPesquisadosArray().length == 0)) {
					...
					if ("nome".equalsIgnoreCase(pesquisa) || "razao".equalsIgnoreCase(pesquisa)) {
						destino = SCLIENTES;
						...
						if (listaClientes.getClientesPesquisadosArray().length == 0) {
							request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
							return mapping.findForward(SCLIENTES);
						}
			...
			} else if ("celular".equals(pesquisa)) {

				ConvergenciaVO convergenciaVO = ConvergenciaVO.Factory.newInstance();
				convergenciaVO.setNrTelefone(request.getParameter("valor").trim());

				convergenciaVO = telaInicialFacadeControl.getLinhasConvergentes(user, convergenciaVO);
				convergenciaVO.setNrTelefone(request.getParameter("valor"));

				pesquisaForm.setLinhasConvergentes(convergenciaVO);
				form.setLinhasConvergentes(convergenciaVO);

				if (convergenciaVO.getLinhaArray().length > 1) {
					destino = "linhasConvergentes";
				} else {
					if (convergenciaVO.getLinhaArray().length == 1) {
						request.setAttribute("idLinhaTelefonica", convergenciaVO.getLinhaArray(0).getIdLinhaTelefonica());
						request.setAttribute("nrLinha", convergenciaVO.getLinhaArray(0).getNrLinha());
					}
					destino = "redireciona";
				}
				...
				if (request.getParameter("pesquisa") != null && "conta".equals(request.getParameter("pesquisa").trim())) {
	...
	protected void carregaParametrosSessao(String nrLinha, String tipoRelacionamento, HttpServletRequest request) throws Exception {
		...
		String idLinhaTelefonica = 
			parametros.getIdLinha() != null
				? parametros.getIdLinha()
				: request.getSession().getAttribute("idLinhaTelefonica") != null
					? (String) request.getSession().getAttribute("idLinhaTelefonica")
					: null;
		...
		// PARAMETROS ENTREGUE PELO SERVICO TUXEDO
		ParametrosVO pTemp = ParametrosVO.Factory.newInstance();
		pTemp = telaInicialFacadeControl.getParametrosVO(user, idLinhaTelefonica, tipoRelacionamento);

		if (ConstantesCRM.SONE.equals(tiForm.getLinha().getIsPortada())) {
		...
		tiForm.getParametros().setNrLinha(nrLinha);
		tiForm.getParametros().setIdUsuario(user);
		tiForm.getParametros().setIdLinha(idLinhaTelefonica);
		...
		request.getSession().removeAttribute("idProspect");
		request.getSession().removeAttribute("idLinhaTelefonica");
		request.getSession().setAttribute(ConstantesCRM.SPARAMETROS, tiForm.getParametros());
	}
	...
	public ActionForward fatura(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (ConstantesCRM.SONE.equals(param.getIdTipoLinha()) || ConstantesCRM.SFIVE.equals(param.getIdTipoLinha()) || ConstantesCRM.SFOUR.equals(param.getIdTipoLinha()) || "7".equals(param.getIdTipoLinha())) {
	...
	public ActionForward loadDados(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (request.getSession().getAttribute("naoCliente") != null) {
				 ...
				 * request.getSession().removeAttribute("naoCliente");
                   String idNaoCliente = "26";
                   telaInicial = TelaInicialVO.Factory.newInstance();
                   telaInicial.addNewTICliente();
				 ...
				 * telaInicial.getTICliente().setNmNome("Não Identificado");
                   ((ParametrosVO) request.getSession().getAttribute(ConstantesCRM.SPARAMETROS)).setIdTipoLinha(ConstantesCRM.SONE);
				 * ((ParametrosVO) request.getSession().getAttribute(ConstantesCRM.SPARAMETROS)).setIdPessoaCliente(idNaoCliente);
				 * ((ParametrosVO) request.getSession().getAttribute(ConstantesCRM.SPARAMETROS)).setIdClienteDePara(idNaoCliente);
				 * ((ParametrosVO) request.getSession().getAttribute(ConstantesCRM.SPARAMETROS)).setIdUsuarioDePara(idNaoCliente);
				 * ((ParametrosVO) request.getSession().getAttribute(ConstantesCRM.SPARAMETROS)).setIdPessoaUsuario(idNaoCliente);
				 * ((ParametrosVO) request.getSession().getAttribute(ConstantesCRM.SPARAMETROS)).setNrConta(ConstantesCRM.SVAZIO);
				 * ((ParametrosVO) request.getSession().getAttribute(ConstantesCRM.SPARAMETROS)).setIdTipoRelacionamento("7");
				 */
			} else {
				// CLIENTE OU USUARIO
				if (request.getSession().getAttribute("idProspect") == null) {
					...
					if (ConstantesCRM.SONE.equals(telaInicial.getTILinha().getInBloqueado())) {
			...
			if (request.getParameter("first") == null) {
				...
				if ((idTipoLinha == 2 || idTipoLinha == 6 /* || idTipoLinha==4 || idTipoLinha==7 */) && "SERVICO".equalsIgnoreCase(request.getParameter("redirect"))) {
					...
					try {
                        ...
                        logLupaLinha.info("TelaInicialController:pesquisarDetalheLinha(" + user + ") [" + nrLinhaFiltro + "]");
					...
					String inMostrarTrafego1005 = (parametroVO != null && parametroVO.getDsValorParametro() != null) ? parametroVO.getDsValorParametro() : ConstantesCRM.SZERO;
					if (reducaoVelocidadeItem1005 != null || reducaoVelocidadeItem1006 != null) {
						...
						if (ConstantesCRM.SONE.equals(inMostrarTrafego1005)) {
							if (reducaoVelocidadeItem1005 != null || reducaoVelocidadeItem1006 != null) {
	...
	public ActionForward modeloAparelho(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				String msgConexao = ConstantesCRM.SVAZIO;
				if ("historico".equals(nmOperacao)) {
				...
				throw new com.indracompany.ws.aparelhoservice.to.ErroInfo(ConstantesCRM.STHREE, msgConexao, Calendar.getInstance(), axisFaultException.getFaultString(),  new BigInteger(ConstantesCRM.SZERO), axisFaultException.getFaultString());

				
			} catch (Exception e) {
				logTelaInicial.error("TelaInicialController:modeloAparelho Exception "  ,e);
				throw new com.indracompany.ws.aparelhoservice.to.ErroInfo(ConstantesCRM.SZERO, "Serviço indisponível no momento. ", Calendar.getInstance(), e.getMessage(),  new BigInteger(ConstantesCRM.SZERO), e.getMessage());
			}
	...
	public ActionForward consultarLinhaPUP(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		logTelaInicial.error("TelaInicialController:consultarLinhaPUP(" + user + ") - [Parametro VO NRLINHA: " + parametrosVO.getNrLinha() + "]");
		...
		try {
			...
			}else if(pesquisaLinhaPUP.getLinhaPUPWSTO()==null){
				pesquisaLinhaPUP.setLinhaPUPWSTO(new LinhaPUPWSTO());
			}

			logTelaInicial.error("TelaInicialController:consultarLinhaPUP(" + user + ") - [" + nrLinha + "]");
	...
	public ActionForward gravarLinhaPUP(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		...
		String DATE_FORMAT = ConstantesCRM.formatoData;
	...
	public ActionForward lupaLinhaAbaHistAparelho(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (nrLinha != null && !ConstantesCRM.SVAZIO.equals(nrLinha)) {
				...
				// telaInicialFacadeControl.getLupaLinhaAbasGSM(user,ConstantesCRM.STWO,idLinha,nrLinha,headerIP,null);
	...
	public ActionForward lupaLinhaAbaParametros(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (nrLinha != null && !ConstantesCRM.SVAZIO.equals(nrLinha)) {
				// LupaLinhaAbasGSMVO lupaLinhaAbasGSMVO =
				// telaInicialFacadeControl.getLupaLinhaAbasGSM(user,ConstantesCRM.SONE,idLinha,nrLinha,headerIP,null);
				// getLupaLinhaAbasGSMForm().setLupaLinhaAbasGSMVO(lupaLinhaAbasGSMVO);
	...
	public ActionForward lupaLinhaAbaParametrosAtualiza(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (nrLinha != null && !ConstantesCRM.SVAZIO.equals(nrLinha)) {
				/*
				 * LupaLinhaAbasGSMVO lupaLinhaAbasGSMVO = telaInicialFacadeControl.getLupaLinhaAbasGSM(user,ConstantesCRM.STHREE,idLinha,nrLinha,headerIP,nmParam);
				 * if(ConstantesCRM.SZERO.equals(lupaLinhaAbasGSMVO.getCodErro())){ 
				 * //lupaLinhaAbasGSMVO = telaInicialFacadeControl.getLupaLinhaAbasGSM(user,ConstantesCRM.SONE,idLinha,nrLinha,headerIP,null);
	...
	public ActionForward desbloqueioAparelhoGsm(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (request.getParameter("imei") != null) {
				...
				getDesbloqueioImei().getDesbloqueioGsmVO().setIdPessoa(ConstantesCRM.STWO.equals(param.getIdTipoRelacionamento()) ? param.getIdPessoaCliente() : param.getIdPessoaUsuario());
	...
	public ActionForward carregaNaoClienteXml(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		tmpVO.setIdTipoRelacionamento("7");
	...
	public ActionForward carregaTelaInicialXml(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (!forward.equals(ConstantesCRM.SVAZIO)) {
			...
			try {
				...
				request.getSession().setAttribute("idLinhaTelefonica", idLinhaTelefonica);
				...
				telaInicial = telaInicialFacadeControl.getTelaInicialById(user, idLinhaTelefonica);
				...
				if (request.getSession().getAttribute("idProspect") != null) {
					String idProspect = request.getSession().getAttribute("idProspect").toString();
					parametros.setIdTipoRelacionamento("6");
				...
				// portabilidade
				if (ConstantesCRM.SONE.equals(telaInicial.getTILinha().getIsPortada())) {
				...
				if (telaInicial.getTILinha() != null) {
					if(telaInicial.getTILinha().getNrLinha().length() == 8 || telaInicial.getTILinha().getNrLinha().length() == 9){
						parametros.setNrLinha(telaInicial.getTILinha().getNrCodArea() + telaInicial.getTILinha().getNrLinha());
					}
					else{
						parametros.setNrLinha(telaInicial.getTILinha().getNrLinha());
					}
				}
				...
				//xmlDadosTI.append("<nrLinhaFormat>").append(StringUtilStatic.mask(telaInicial.getTILinha().getNrLinha(), "(##)####-####")).append("</nrLinhaFormat>");
				...
				logTelaInicial.error("TelaInicialController:carregaTelaInicialXml(" + user + ") - [" + xmlDadosTI.toString() + "]");
				request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
				response.setContentType(ConstantesCRM.SContentType);
				BufferedOutputStream bufferedOutputStream = new BufferedOutputStream(response.getOutputStream());
				bufferedOutputStream.write(xmlDadosTI.toString().getBytes(ConstantesCRM.SISO));
	...
	public ActionForward getHistoricoServicos(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
			if (historicoServicos.getErrorAPI() != null && historicoServicos.getErrorAPI().equalsIgnoreCase(ConstantesCRM.SONE)) {
				request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
	...
	/*private ListaTrackingAparelhoVO agrupaListaTracking(ListaTrackingAparelhoVO listaTracking) {
		...
		for (int i = 0; i < listaTracking.getListaPedidoAparelhosVOArray().length; i++) {
			...
			if (ConstantesCRM.SONE.equals(isOVParcial) || ConstantesCRM.STWO.equals(isOVParcial)) {
	...
	protected String formataData(String data) {
		if (data != null && !ConstantesCRM.SVAZIO.equals(data)) {
			...
			try {
				...
				try {
					dateFormat = new SimpleDateFormat(ConstantesCRM.formatoAnoMesDiaHoraMinutoSegundo);//"yyyy-mm-dd hh:mm:ss"
					Date dtUtil = new Date(data);
					data = dateFormat.format(dtUtil);
				}catch (Exception ex) {
	...
	public static class PesquisaForm extends ActionForm {
		...
		private String pageNumber;
		private String hasNext;
		private ConvergenciaVO linhasConvergentes;
		...
		public ConvergenciaVO getLinhasConvergentes() {
    	return this.linhasConvergentes;
    }

		public void setLinhasConvergentes(ConvergenciaVO arg) {
    	this.linhasConvergentes = arg;
    }
	...
	public static class LupaLinhaForm extends ActionForm {
		...
		public String getFormPaginaAtual() {
			if (this.formPaginaAtual == null) {
				this.formPaginaAtual = ConstantesCRM.SZERO;
			}
