package cliente.TelaInicial.DetalheCliente.customer;
...
public class CustomerController extends AbstractAction {
	...
	private static final long serialVersionUID = -6550276113986399838L;
	...
	public ActionForward begin(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			logLupaCliente.debug("CustomerController:begin(" + user + ") >> INICIALIZADO");
			...
			logLupaCliente.debug("CustomerController:begin(" + user + ") >> FINALIZADO");
			request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
			return mapping.findForward(ConstantesCRM.SUCCESS);
		} catch (Exception e) {
			logLupaCliente.error("CustomerController:begin(" + user + ") - [" + e.getMessage() + "]", e);
	...
	public ActionForward loadOutrosDados(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			user = ConstantesCRM.getCurrentUser(request.getSession());
			logLupaCliente.debug("CustomerController:loadOutrosDados(" + user + ") >> INICIALIZADO");
			manterDadosForm = new ManterDadosForm();
			...
			logLupaCliente.debug("CustomerController:loadOutrosDados(" + user + ") >> FINALIZADO");
			request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
			return mapping.findForward(ConstantesCRM.SUCCESS);

		} catch (Exception e) {
			logLupaCliente.error("CustomerController:loadOutrosDados(" + user + ") - [" + e.getMessage() + "]", e);
			FormError formError = globalApp.buildFormError(e, URLErro.getURLErro(request.getSession()));
			formError.setTarget(ConstantesCRM.FRAMEAPP);
			request.setAttribute(ConstantesCRM.SFORMERROR, formError);
			request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
	...
	public ActionForward salvar(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			user = ConstantesCRM.getCurrentUser(request.getSession());
			logLupaCliente.debug("CustomerController:salvar(" + user + ") >> INICIALIZADO");
			String idSubAssunto = request.getParameter("idSubAssunto");
			logLupaCliente.debug("CustomerController:salvar(" + user + ") >> idSubAssunto: " + idSubAssunto);
			...
			logLupaCliente.debug("CustomerController:salvar(" + user + ") >> FINALIZADO");
			request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
			return mapping.findForward(ConstantesCRM.SUCCESS);

		} catch (Exception e) {
			logLupaCliente.error("CustomerController:salvar(" + user + ") - [" + e.getMessage() + "]", e);
			FormError formError = globalApp.buildFormError(e, URLErro.getURLErro(request.getSession()));
			formError.setTarget(ConstantesCRM.FRAMEAPP);
			request.setAttribute(ConstantesCRM.SFORMERROR, formError);
			request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
	...
	public ActionForward loadAtributos(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		try {
			ManterDadosForm form = (ManterDadosForm) formParam;
			user = ConstantesCRM.getCurrentUser(request.getSession());
			logLupaCliente.debug("CustomerController:loadAtributos(" + user + ") >> INICIALIZADO");
			if (request.getParameter("start") == null) {
				...
				if (manterDadosForm.getAtributos().length > 0 && manterDadosForm.getAtributos()[0].getIdAtributo() != null) {
					logLupaCliente.debug("CustomerController:loadAtributos(" + user + ") >> FINALIZADO (PERGUNTAS)");
					request.setAttribute("manterDadosForm", manterDadosForm);
					request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
					return mapping.findForward(ConstantesCRM.SUCCESS);
				} else {
					logLupaCliente.debug("CustomerController:loadAtributos(" + user + ") >> FINALIZADO (NULO)");
					request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
			...
			logLupaCliente.error("CustomerController:loadAtributos(" + user + ") - [" + e.getMessage() + "]", e);
			FormError formError = globalApp.buildFormError(e, URLErro.getURLErro(request.getSession()));
			formError.setTarget(ConstantesCRM.FRAMEAPP);
			request.setAttribute(ConstantesCRM.SFORMERROR, formError);
			request.setAttribute(ConstantesCRM.SKEY_REQUEST_PAGEFLOW, this);
	...
	public static class ManterDadosForm extends ActionForm {
		...
		private static final long serialVersionUID = 4885611462293873782L;
