package cliente.TelaInicial.DetalheFatura.FaturaOnlineOptin32;
...
import br.com.vivo.fo.ctrls.VOLTAV.fatura.FaturaFacade;
...
public class FaturaOnlineOptin32Controller extends AbstractAction {
    ...
    private FaturaOnlineForm    faturaOnlineForm;
	...
	public ActionForward begin(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			log.debug("[FaturaOnlineOptin32Controller.begin] isManutencao() " + form.isManutencao());
			if (form.isManutencao()) {
            ...
            log.debug("[FaturaOnlineOptin32Controller.begin] InTipoPessoa" + getParametrosVO(request).getInTipoPessoa());
			...
			if (axisFaultException instanceof ErroInfo) {
				...
				if (Integer.parseInt(erroInfo.getCodigo()) == STATUS_EMPROCESSAMENTO) {
					...
					if (getFaturaOnlineForm().isManutencao()) {
                        ...
                        if (("PJ".equalsIgnoreCase(getParametrosVO(request).getInTipoPessoa()) && !getFaturaOnlineForm().isFaturaOnlineAtiva()) || (getFaturaOnlineForm().isManutencao())
                                && "PJ".equalsIgnoreCase(getParametrosVO(request).getInTipoPessoa())) {
							getDadosGestorConta(request);
						}
	...
	private void getDadosGestorConta(HttpServletRequest request) throws Exception {
        ...
        DocumentoVO[] documento = telaInicialFacade.getLupaClienteVO(idUsuario, getParametrosVO(request).getIdPessoaCliente(), getParametrosVO(request).getNrLinha(), getParametrosVO(request).getIdConta(), idTipoLinha).getDadosAbaLupaCliente()
                .getDocumentoVOArray();
		...
		try {
			...
			if (getFaturaOnlineForm().isServicoEnvioFaturaPorEmail() || getFaturaOnlineForm().isAvisoFaturaDisponivelPorEmail()) {
				...
				if (!getFaturaOnlineForm().isServicoEnvioFaturaPorEmail() && getFaturaOnlineForm().isAvisoFaturaDisponivelPorEmail()) {
                    ...
                    faturaFacade.ativarSomenteAvisoDisponibilidadeEmail(idUsuario, getParametrosVO(request).getNrLinha(), dsEmail, ConstantesCRM.SVAZIO, new Integer(getParametrosVO(request).getIdUfOperadora()), isPrePago,
                            "PJ".equalsIgnoreCase(getParametrosVO(request).getInTipoPessoa()) ? true : false, getFaturaOnlineForm().getNrConta());

				} else {
                    faturaFacade.ativaEmail(idUsuario, getParametrosVO(request).getNrLinha(), dsEmail, ConstantesCRM.SVAZIO, new Integer(getParametrosVO(request).getIdUfOperadora()), isPrePago,
                            "PJ".equalsIgnoreCase(getParametrosVO(request).getInTipoPessoa()) ? true : false, getFaturaOnlineForm().getNrConta());
				}
	...
	private String registraPalitagem(String idUsuario, String cdParametro, String msgRetorno, HttpServletRequest request) {
		...
		if (!cdParametro.equals(ConstantesCRM.SVAZIO)) {
			...
			try {
                nrProtocolo = ClienteUtils.registrarPalitagem(idUsuario, getParametrosVO(request), getParametrosVO(request).getIdLinha(), getParametrosVO(request).getNrLinha(), nmPath, ConstantesCRM.SVAZIO);
			} catch (Exception e) {
	...
	public ActionForward manageContaOnline(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		log.info("[FaturaOnlineOptin32Controller:manageContaOnline] Inicio ");
		FaturaOnlineForm form = (FaturaOnlineForm) formParam;
		String idUsuario = ConstantesCRM.getCurrentUser(request.getSession());
		...
		log.info("[FaturaOnlineOptin32Controller:manageContaOnline] getOperacao " + getFaturaOnlineForm().getOperacao());
		...
		try {
			...
			if ("getServicos".equals(getFaturaOnlineForm().getOperacao())) {
				...
				log.info("[FaturaOnlineOptin32Controller:manageContaOnline] isAvisoFaturaDisponivelPorEmail " + form.isAvisoFaturaDisponivelPorEmail());
				log.info("[FaturaOnlineOptin32Controller:manageContaOnline] isServicoEnvioFaturaPorEmail " + form.isServicoEnvioFaturaPorEmail());
				if (form.isAvisoFaturaDisponivelPorEmail() || form.isServicoEnvioFaturaPorEmail()) {
					// Verificação de e-mails de um cliente PF
					log.info("[FaturaOnlineOptin32Controller:manageContaOnline] isServicoEnvioFaturaPorEmail " + getParametrosVO(request).getInTipoPessoa());
					if ("PF".equalsIgnoreCase(getParametrosVO(request).getInTipoPessoa())) {
						getFaturaOnlineForm().setListaEmails(null);
	...
	public ActionForward manageCadastroEmail(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (form.getCadastroEmailPF().getDsContato() == null || ConstantesCRM.SVAZIO.equals(form.getCadastroEmailPF().getDsContato())) {
            ...
            telaInicialFacade.setSalvarNovaComunicacao(idUsuario, dadosGravacaoEmail);
