package admsistemas.admArvoreBaixa.dadosBasicos.abaFiltros;
...
public class AbaFiltrosController extends AbstractAction {
	...
	private static final long serialVersionUID = -6014591789996461816L;

	@EJB
	private br.com.vivo.fo.ctrls.admsistemas.filtrosBxa.FiltrosBxa controlFiltro;
	...
	public ActionForward begin(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// id=request.getParameter("idBaixa");

			formFiltro.setIdBaixa(request.getParameter("idBaixa"));
			formFiltro.setNmBaixa(request.getParameter("nmBaixa"));

			AdmIdBaixaVO admIdBaixaVO = AdmIdBaixaVO.Factory.newInstance();
			AdmFiltrosVO admFiltrosVO = AdmFiltrosVO.Factory.newInstance();
			admIdBaixaVO.setIdBaixa(formFiltro.getIdBaixa());
			admFiltrosVO = controlFiltro.listaFiltros(admIdBaixaVO, ConstantesCRM.getCurrentUser(request.getSession()));
	...
	public ActionForward salvarFiltros(ActionMapping mapping, ActionForm formParam, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			admFiltrosVO.setIdBaixa(form.getIdBaixa());
			...
			formFiltro.setIdBaixa(form.getIdBaixa());
			...
			formFiltro.setMsgError(twe.getMessage().substring(twe.getMessage().indexOf(']') + 1));
	...
	public static class FormFiltro extends ActionForm {
		...
		private static final long serialVersionUID = 7690518594335299444L;
