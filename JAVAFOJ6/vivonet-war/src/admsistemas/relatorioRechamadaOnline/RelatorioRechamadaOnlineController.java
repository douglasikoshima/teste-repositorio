package admsistemas.relatorioRechamadaOnline;
...
public class RelatorioRechamadaOnlineController extends AbstractAction {
	...
	private void buscar (String dataInicio, String dataTermino) {
        try{
            ...
            query.append(" select to_char(dad.dtabertura, 'hh24') || 'h às ' ||                                         	   "); 
            query.append("        (to_number(to_char(dad.dtabertura, 'hh24'))) || 'h59min' as intervalo,                       ");
            ...
            query.append("                where to_char(dtabertura, 'yyyymmddhh24') between                                    "); 
            ...
            query.append("                                                                                                     "); 
            query.append("        (select count(1) t, to_char(dtabertura, 'hh24') d                                            "); 
            query.append("           from atendimento.dadosrecontato                                                           "); 
            query.append("          where to_char(dtabertura, 'yyyymmddhh24') between                                          "); 
            query.append("                to_char(sysdate, 'yyyymmdd') || '00' and                                             "); 
            query.append("                to_char(sysdate, 'yyyymmdd') ||                                                      "); 
            query.append("                lpad(to_number(to_char(sysdate, 'hh24') - 1), 2, 0)                                  "); 
            query.append("          group by to_char(dtabertura, 'hh24')) subtotal,                                            "); 
            query.append("                                                                                                     ");         
            ...
            query.append("    and to_char(dad.dtabertura, 'hh24') = subtotal.d                                                 "); 
            query.append("    and to_char(dad.dtabertura, 'yyyymmdd') = to_char(sysdate, 'yyyymmdd')                           "); 
            ...
            if (!dataInicio.equals("-1") && !dataTermino.equals("-1")) {                                
                if (dataInicio.equals(dataTermino)) {
                    query.append(" and to_char(dad.dtabertura, 'HH24') = ").append(Integer.parseInt(dataInicio));
                } else {        
                    query.append(" and to_char(dad.dtabertura, 'HH24') >= ").append(Integer.parseInt(dataInicio));
                    query.append(" and to_char(dad.dtabertura, 'HH24') <= ").append(Integer.parseInt(dataTermino));
                }
            }            
            
            query.append("  group by to_char(dad.dtabertura, 'hh24'),                           "); 
            query.append("           mot.nmmotivorecontato,                                     "); 
            query.append("           sub.nmsubmotivorecontato,                                  "); 
            query.append("           subtotal.t,                                                "); 
            query.append("           total.a                                                    "); 
            query.append("                                                                      "); 
            query.append("  order by to_char(dad.dtabertura, 'hh24') desc, count(1) desc        "); 
