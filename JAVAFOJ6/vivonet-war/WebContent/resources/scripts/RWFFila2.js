...
function linhaSel(linha) {
    ...
    eval(zsa)
    fila = escape("/FrontOfficeWeb/extworkflw/AtendimentoFila/begin.do");
    inCRI  = f.elements["inCRI"].value;
    inResp = f.elements["inResp"].value;
    if(inResp=='1'){
        fila = escape("/FrontOfficeWeb/extworkflw/AtendimentoFila/beginResp.do");
    }
    if(inCRI=='1'||inCRI=='2'){
        fila = escape("/FrontOfficeWeb/extworkflw/AtendimentoFila/beginCRI.do");
    }
    if (f.inMirrorRC!=null){
        if (f.inMirrorRC.value=="1"){
...
function habilitarCampos(){
    ...
    // document.getElementById("imgLupa").href = "/FrontOfficeWeb/extworkflw/AtendimentoFila/obterArvoreContato.do";
