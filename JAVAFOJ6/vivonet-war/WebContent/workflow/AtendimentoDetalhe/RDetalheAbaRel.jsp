...
<table width="98%" align="right">
    ...
    <tr valign="top">
        <logic:notEmpty name="atdVO" property="idAtendimentoOrigem">
            ...
            <td onclick="exibeLinkOrigem('<bean:write name="atdVO" property="idAtendimentoOrigem"/>');" style="cursor:hand;">Clique aqui</td>
            <%}else{%>
            <td colspan="2"></td>
            <%}%>
        </logic:notEmpty>
...
function exibeLinkOrigem(a){
    ...
    if($(fdo)){
        ...
        if(top.frameApp.$('iframeAtendimento')){
            eval(zsa);
            parent.$('iframeAtendimento').$(fdo).src="begin2.do?idAtendimento="+a+"&inMenu=N&fila="+fl+"&qtd="+qtd;
            parent.$('iframeAtendimento').$(ddo).style.display='';
        }
    } */
}
</logic:notEmpty>
