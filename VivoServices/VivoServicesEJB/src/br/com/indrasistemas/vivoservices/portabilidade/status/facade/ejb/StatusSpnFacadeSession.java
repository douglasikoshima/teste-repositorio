...
/**
 * Camada Session para StatusSpnFacade.
 * @xdoclet-generated at ${TODAY}
 * @copyright The XDoclet Team
 * @author XDoclet
 * @version ${version}
 */
public class StatusSpnFacadeSession
   extends br.com.indrasistemas.vivoservices.portabilidade.status.facade.ejb.StatusSpnFacadeBean
   implements javax.ejb.SessionBean
{
   private static final long serialVersionUID = 1L;

   private static final Log logger = LogFactory.getLog(StatusSpnFacadeSession.class);
   ...
   public br.com.indrasistemas.vivoservices.portabilidade.status.to.RespostaTO validaStatusSpn( br.com.indrasistemas.framework.service.to.RequestInfoTO requestInfo,java.lang.String nrLinha ) throws br.com.indrasistemas.framework.service.BusinessException	{
      //Verificação do parâmetro RequestInfoTO
      if(requestInfo == null)	{
        String errMsg = "O parâmetro requestInfo não pode ser nulo. O método validaStatusSpn não pode ser executado.";
        logger.error(errMsg);
      	throw new NullPointerException(errMsg);
      }
      //Verificação do parâmetro RequestInfoTO.userName
      if(requestInfo.getUserName() == null)	{
        String errMsg = "O parâmetro RequestInfoTO.userName não pode ser nulo. O método validaStatusSpn não pode ser executado.";
        logger.error(errMsg);
      	throw new NullPointerException(errMsg);
      }
      ...
      appRegister.register("br.com.indrasistemas.vivoservices.portabilidade.status.facade.ejb.StatusSpnFacadeBean", "validaStatusSpn");
      //Se nulo é a primeira interação da cadeia de chamadas.
      if(requestInfo.getServiceName() == null)	{
        requestInfo.setServiceName("br.com.indrasistemas.vivoservices.portabilidade.status.facade.ejb.StatusSpnFacadeBean.validaStatusSpn");
      }
      //Início token audit.
      ...
      if(tokenAudit.isEnabled())  {
          tokenAudit.setServiceName("StatusSpnFacade.validaStatusSpn");
      ...
      if(standardAudit.isEnabled())  {
          standardAudit.setServiceName("StatusSpnFacade.validaStatusSpn");
          standardAudit.putParameter(requestInfo);
          standardAudit.putParameter(nrLinha);
      }      

      //Call
      br.com.indrasistemas.vivoservices.portabilidade.status.to.RespostaTO result = null;
      try	{
	      result = super.validaStatusSpn( requestInfo,nrLinha );
