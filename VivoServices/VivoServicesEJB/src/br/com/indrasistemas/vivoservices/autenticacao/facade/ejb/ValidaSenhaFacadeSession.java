...
/**
 * Camada Session para ValidaSenhaFacade.
 * @xdoclet-generated at ${TODAY}
 * @copyright The XDoclet Team
 * @author XDoclet
 * @version ${version}
 */
public class ValidaSenhaFacadeSession
   extends br.com.indrasistemas.vivoservices.autenticacao.facade.ejb.ValidaSenhaFacadeBean
   implements javax.ejb.SessionBean
{
   private static final long serialVersionUID = 1L;

   private static final Log logger = LogFactory.getLog(ValidaSenhaFacadeSession.class);
   ...
   public br.com.indrasistemas.vivoservices.autenticacao.to.ResultadoValidarSenhaTO validaSenha( br.com.indrasistemas.framework.service.to.RequestInfoTO requestInfo,br.com.indrasistemas.vivoservices.autenticacao.to.DadosValidaSenhaTO dados ) throws br.com.indrasistemas.framework.service.BusinessException	{
      ...
      if(requestInfo == null)	{
        String errMsg = "O parâmetro requestInfo não pode ser nulo. O método validaSenha não pode ser executado.";
        logger.error(errMsg);
      	throw new NullPointerException(errMsg);
      }
      //Verificação do parâmetro RequestInfoTO.userName
      if(requestInfo.getUserName() == null)	{
        String errMsg = "O parâmetro RequestInfoTO.userName não pode ser nulo. O método validaSenha não pode ser executado.";
        logger.error(errMsg);
      	throw new NullPointerException(errMsg);
      }
      ...
      appRegister.register("br.com.indrasistemas.vivoservices.autenticacao.facade.ejb.ValidaSenhaFacadeBean", "validaSenha");
      //Se nulo é a primeira interação da cadeia de chamadas.
      if(requestInfo.getServiceName() == null)	{
        requestInfo.setServiceName("br.com.indrasistemas.vivoservices.autenticacao.facade.ejb.ValidaSenhaFacadeBean.validaSenha");
      }
      //Início token audit.
      ...
      if(tokenAudit.isEnabled())  {
          tokenAudit.setServiceName("ValidaSenhaFacade.validaSenha");
      ...
      if(standardAudit.isEnabled())  {
          standardAudit.setServiceName("ValidaSenhaFacade.validaSenha");
          standardAudit.putParameter(requestInfo);
          standardAudit.putParameter(dados);
      }      

      //Call
      br.com.indrasistemas.vivoservices.autenticacao.to.ResultadoValidarSenhaTO result = null;
      try	{
	      result = super.validaSenha( requestInfo,dados );
