...
import br.com.indrasistemas.vivoservices.atendimento.gestor.application.AlertaSaidaAS;
import br.com.indrasistemas.vivoservices.atendimento.gestor.to.AlertaSaidaTO;

/**
 * @ejb.bean name="AlertaSaidaFacade" display-name="AlertaSaidaFacade"
 *           description="" jndi-name="ejb/vivoservices/AlertaSaidaFacade"
 *           type="Stateless"
 *           local-jndi-name="ejb/vivoservices/local/AlertaSaidaFacade"
 *           view-type="both"
 * @ejb.transaction type="Required"
 * @weblogic.enable-call-by-reference True
 */
public class AlertaSaidaFacadeBean implements SessionBean {
     ...
     */
    private static final Log  logger           = LogFactory.getLog(AlertaSaidaFacadeBean.class);

    /** The session context */
    private SessionContext    context;

    /**
     * @J2EE_METHOD -- AlertaSaidaFacadeBean
     */
    public AlertaSaidaFacadeBean() {
     ...
     */
    public AlertaSaidaTO consultar(RequestInfoTO requestInfo, Long cdAreaRegistro, Long nrLinha) throws BusinessException {
        AlertaSaidaTO result = null;
        try{
            AlertaSaidaAS as = new AlertaSaidaAS(this.getSessionContext());
            result = as.consultar(requestInfo, cdAreaRegistro, nrLinha);
            if(logger.isDebugEnabled()){
                logger.debug("AS OK");
            }
        }catch(Exception e){
            logger.error("ERROR::AlertaSaidaFacadeBean::consultar", e);
