...
import br.com.indrasistemas.vivoservices.atendimento.palitagem.application.PalitagemAS;
import br.com.indrasistemas.vivoservices.atendimento.palitagem.to.PalitagemTO;

/**
 * @ejb.bean name="PalitagemFacade" display-name="PalitagemFacade"
 *           description="" jndi-name="ejb/vivoservices/PalitagemFacade"
 *           type="Stateless"
 *           local-jndi-name="ejb/vivoservices/local/PalitagemFacade"
 *           view-type="both"
 * @ejb.transaction type="Required"
 * @weblogic.transaction-descriptor trans-timeout-seconds="300"
 * @weblogic.enable-call-by-reference True
 */
public class PalitagemFacadeBean implements SessionBean {
     ...
     */
    private static final Log  logger           = LogFactory.getLog(PalitagemFacadeBean.class);

    /** The session context */
    private SessionContext    context;

    /**
     * @J2EE_METHOD -- PalitagemFacadeBean
     */
    public PalitagemFacadeBean() {
    ...
    public PalitagemTO registrarPalito(RequestInfoTO requestInfo, PalitagemTO to) throws BusinessException {
        ...
        try{
            PalitagemAS as = new PalitagemAS(this.getSessionContext());
            result = as.registrarPalito(requestInfo, to);
            if(logger.isDebugEnabled()){
                logger.debug("AS OK");
            }
        }catch(Exception e){
            logger.error("ERROR::PalitagemFacadeBean::registrarPalito", e);
