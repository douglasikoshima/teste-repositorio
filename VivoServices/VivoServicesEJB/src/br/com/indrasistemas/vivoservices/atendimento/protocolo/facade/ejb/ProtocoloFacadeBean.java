...
import br.com.indrasistemas.vivoservices.atendimento.protocolo.application.ProtocoloAS;
import br.com.indrasistemas.vivoservices.atendimento.protocolo.to.ProtocoloTO;

/**
 * @ejb.bean name="ProtocoloFacade" display-name="ProtocoloFacade"
 *           description="" jndi-name="ejb/vivoservices/ProtocoloFacade"
 *           type="Stateless"
 *           local-jndi-name="ejb/vivoservices/local/ProtocoloFacade"
 *           view-type="both"
 * @ejb.transaction type="Required"
 * @weblogic.enable-call-by-reference True
 */
public class ProtocoloFacadeBean implements SessionBean {
     ...
     */
    private static final Log  logger           = LogFactory.getLog(ProtocoloFacadeBean.class);

    /** The session context */
    private SessionContext    context;

    /**
     * @J2EE_METHOD -- ProtocoloFacadeBean
     */
    public ProtocoloFacadeBean() {
     ...
     * @J2EE_METHOD -- aberturaProtocolo
     */
    public ProtocoloTO aberturaProtocolo(RequestInfoTO requestInfo, ProtocoloTO to) throws BusinessException {
        ProtocoloTO result = null;
        try{
            ProtocoloAS as = new ProtocoloAS(this.getSessionContext());
            result = as.aberturaProtocolo(requestInfo, to);
            if(logger.isDebugEnabled()){
                logger.debug("AS OK");
            }
        }catch(Exception e){
            if(logger.isDebugEnabled()){
                logger.debug("Ocorreu um erro ao Chamar o AS. (" + e.getMessage() + ")");
            }
     ...
     * @return
     * @throws BusinessException
     * @J2EE_METHOD -- fechamentoProtocolo
     */
    public ProtocoloTO fechamentoProtocolo(RequestInfoTO requestInfo, ProtocoloTO to) throws BusinessException {
        ProtocoloTO result = null;
        try{
            ProtocoloAS as = new ProtocoloAS(this.getSessionContext());
            result = as.fechamentoProtocolo(requestInfo, to);
            if(logger.isDebugEnabled()){
                logger.debug("AS OK");
            }
        }catch(Exception e){
