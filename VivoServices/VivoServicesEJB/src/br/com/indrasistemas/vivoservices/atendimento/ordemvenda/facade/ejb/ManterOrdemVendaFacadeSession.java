...
/**
 * Camada Session para ManterOrdemVendaFacade.
 * @xdoclet-generated at ${TODAY}
 * @copyright The XDoclet Team
 * @author XDoclet
 * @version ${version}
 */
public class ManterOrdemVendaFacadeSession
   extends br.com.indrasistemas.vivoservices.atendimento.ordemvenda.facade.ejb.ManterOrdemVendaFacadeBean
   implements javax.ejb.SessionBean
{
   private static final long serialVersionUID = 1L;

   private static final Log logger = LogFactory.getLog(ManterOrdemVendaFacadeSession.class);
   ...
   public br.com.indrasistemas.vivoservices.atendimento.ordemvenda.to.PalitagemTO registrarPalito( br.com.indrasistemas.framework.service.to.RequestInfoTO requestInfo,br.com.indrasistemas.vivoservices.atendimento.ordemvenda.to.PalitagemTO to ) throws br.com.indrasistemas.framework.service.BusinessException	{
      //Verificação do parâmetro RequestInfoTO
      if(requestInfo == null)	{
        String errMsg = "O parâmetro requestInfo não pode ser nulo. O método registrarPalito não pode ser executado.";
        logger.error(errMsg);
      	throw new NullPointerException(errMsg);
      }
      //Verificação do parâmetro RequestInfoTO.userName
      if(requestInfo.getUserName() == null)	{
        String errMsg = "O parâmetro RequestInfoTO.userName não pode ser nulo. O método registrarPalito não pode ser executado.";
        logger.error(errMsg);
      	throw new NullPointerException(errMsg);
      }
      ...
      appRegister.register("br.com.indrasistemas.vivoservices.atendimento.ordemvenda.facade.ejb.ManterOrdemVendaFacadeBean", "registrarPalito");
      //Se nulo é a primeira interação da cadeia de chamadas.
      if(requestInfo.getServiceName() == null)	{
        requestInfo.setServiceName("br.com.indrasistemas.vivoservices.atendimento.ordemvenda.facade.ejb.ManterOrdemVendaFacadeBean.registrarPalito");
      }
      //Início token audit.
      ...
      if(tokenAudit.isEnabled())  {
          tokenAudit.setServiceName("ManterOrdemVendaFacade.registrarPalito");
      ...
      if(standardAudit.isEnabled())  {
          standardAudit.setServiceName("ManterOrdemVendaFacade.registrarPalito");
          standardAudit.putParameter(requestInfo);
          standardAudit.putParameter(to);
      }      

      //Call
      br.com.indrasistemas.vivoservices.atendimento.ordemvenda.to.PalitagemTO result = null;
      try	{
