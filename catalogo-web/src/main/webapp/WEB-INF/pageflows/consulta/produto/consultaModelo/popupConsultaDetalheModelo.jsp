...
<html:form action="/br/com/vivo/catalogoPRS/pageflows/consulta/produto/consultaModelo/abrirpopupTecnologiaModelo.do">
		...
		<div class="titulo">${nmFabricante}&nbsp;${modelo.nmModelo}
			<c:if test="${not empty ConsultaModeloForm.listaTecnologia}">
				<logic:iterate id="tecnologia" property="listaTecnologia" name="ConsultaModeloForm">
					- ${tecnologia.nmTecnologia}
					<c:if test="${not empty ConsultaModeloForm.listaTipoFrequencia}">
 						<logic:iterate id="tipoFrequencia" property="listaTipoFrequencia" name="ConsultaModeloForm">
							: ${tipoFrequencia.nmTipoFrequencia} 
							<c:if test="${not empty ConsultaModeloForm.listaValorFrequencia}">
								<logic:iterate id="valorFrequencia" property="listaValorFrequencia" name="ConsultaModeloForm" indexId="index">
				...
				<td width="370">	
					...
					<jsp:include page="/templates/box_pre.jsp"/>
							<div class="fleft" style="width:120px; height:200px;">
								...
								<c:if test="${not empty ConsultaModeloForm.multimidiaList}">																
	 								<logic:iterate id="multimidia" property="multimidiaList" name="ConsultaModeloForm" indexId="index" >
											<c:if test="${index > 0}">
											...
											<div id="multimidia_${index}" style="display:${display};" align="center">
												<div class="foto-moldura">
													...
													<c:if test="${ConsultaModeloForm.caminho_link_imagens_modelo != null}">
														<c:set var="image" value="${ConsultaModeloForm.caminho_link_imagens_modelo}"/>
													</c:if>
												...
												<div align="center" style="margin-top:5px;">
													...
													<c:choose>
														<c:when test="${ fn:length(ConsultaModeloForm.multimidiaList) > (index +1) }">
							...
							<div class="fleft vertical-scroll" style="width:170px;height:200px;overflow:auto;" align="left">
								<div align="left">
									<ul class="dados-list" style="margin-left:0px">
										<c:if test="${not empty ConsultaModeloForm.caracteristicaList}">																				
	 										<logic:iterate id="caracteristic" property="caracteristicaList" name="ConsultaModeloForm">
												<li>${caracteristic.nmCaracteristica}															
													<c:if test="${not empty ConsultaModeloForm.listaValorCaracteristica}">																
														<logic:iterate id="valorCaracteristica" property="listaValorCaracteristica" name="ConsultaModeloForm">	
				...
				<c:set var="nomeBox" scope="application"  value="Código do Produto"/>
					<jsp:include page="/templates/box_pre.jsp"/>
						<div style="height:200px;">	
							<c:if test="${not empty ConsultaModeloForm.sistemaProdutoList}">
								<logic:iterate id="listSistProd" property="sistemaProdutoList" name="ConsultaModeloForm" >															
