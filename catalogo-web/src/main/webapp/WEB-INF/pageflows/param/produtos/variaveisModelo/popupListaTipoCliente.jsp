...
<netui-temp:template templatePage="/templates/popupPadrao.jsp">
	<netui-temp:section name="conteudo">
		<div id="popup_selecionar_tipo_cliente" style="width: 100%">
			<c:set var="nomeBox" scope="application"  value="Tipo de Clientes"/>
					...
					<div style="padding: 10px 0px 0xp 10px; float: left; width: 100%; height: 280px; overflow-y: auto;">
						<form>
							<table class="tabConteudoGeral" width="100%">
								...
								<netui:checkBoxGroup dataSource="pageFlow.tipoClienteSelecionados" optionsDataSource="${tipoCliente}" repeater="true">
								<tr onmouseover="$(this).className='mouseOver'" onmouseout="$(this).className='mouseOut'">
									<td width=10px">
										<netui:checkBoxOption onClick="selecionaCheckBoxPopup('hidden_lista_tipo_cliente', this)" tagId="cliente_cb_${container.item.idTipoCliente}" value="${container.item.idTipoCliente}" styleClass="semBorda checkbox"/>
									</td>
									<td nowrap="nowrap">
										<c:choose>
											<c:when test="${fn:toUpperCase(container.item.nmTipoCliente) eq 'PF' or fn:toUpperCase(container.item.nmTipoCliente) eq 'PESSOA FÍSICA' or fn:toUpperCase(container.item.nmTipoCliente) eq 'PESSOA FISICA'}">
												<label for="cliente_cb_${container.item.idTipoCliente}">${'Pessoa Física'}</label>
											</c:when>
											<c:otherwise>
												<label for="cliente_cb_${container.item.idTipoCliente}">${'Pessoa Jurídica'}</label>
											</c:otherwise>
										</c:choose>
									</td>
								</tr>
