...
<netui-temp:template templatePage="/templates/popupConfirmacao.jsp">
	...
	<netui-temp:section name="conteudo">
		...
		<br clear="all"/>
		<br clear="all"/>
		<netui:form action="apagarValorCaracteristica">
		<div style="width:70%;">
			<c:if test="${modelos_afetados != null}">
			...
			<netui:hidden dataSource="actionForm.idEntidade" dataInput="${id_valor_caracteristica}"/>
		</div>
		<br clear="all"/>
		<input type="button" onclick="send(this, null, 'popup2', 'div_erros_popup_confirmacao');" class="btNavegacao74" value="Sim" title="${bundle.default['catalogo.popupApagarValorCaracteristica.Sim']}"/>
		</netui:form>
		&nbsp;
		<input type="button" onclick="fecharPopup('popup2');" class="btNavegacao74" value="Não" title="${bundle.default['catalogo.popupApagarValorCaracteristica.Nao']}"/>
		<br/><br/>
	</netui-temp:section>
