	...
	<jsp:attribute name="headScripts">
		...
		<script>
			...
			function editar(id) {
				...
				document.getElementById("associacaoModeloRestricaoForm").action = "/catalogo/br/com/vivo/catalogoPRS/pageflows/param/modelorestricaoconsulta/edit.do#contentForm";
				document.getElementById("associacaoModeloRestricaoForm").submit();
			}

			function clearPage() {
				document.location.href = "/catalogo/br/com/vivo/catalogoPRS/pageflows/param/modelorestricaoconsulta/ConsultaModeloRestricaoAction.do";
			}	

			function search() {
				if (validarFormSearch()) {
					document.getElementById("associacaoModeloRestricaoForm").action = "/catalogo/br/com/vivo/catalogoPRS/pageflows/param/modelorestricaoconsulta/search.do";
					document.getElementById("associacaoModeloRestricaoForm").submit();
				}
			}
			
			function carregar() {
				document.getElementById("associacaoModeloRestricaoForm").action = "/catalogo/br/com/vivo/catalogoPRS/pageflows/param/modelorestricaoconsulta/carregar.do"
				document.getElementById("associacaoModeloRestricaoForm").submit();
			}
			...
			function save() {
				...
				if (validarForm()) {
					...
					document.getElementById("associacaoModeloRestricaoForm").action = "/catalogo/br/com/vivo/catalogoPRS/pageflows/param/modelorestricaoconsulta/save.do";
					document.getElementById("associacaoModeloRestricaoForm").submit();
				}
			}
			...
			function carregarTipoRestricao() {
				...
				document.getElementById("associacaoModeloRestricaoForm").action = "/catalogo/br/com/vivo/catalogoPRS/pageflows/param/modelorestricaoconsulta/carregarTipoRestricao.do";
				document.getElementById("associacaoModeloRestricaoForm").submit();
			}
		...
		<html:form action="/br/com/vivo/catalogoPRS/pageflows/param/modelorestricaoconsulta/search.do" styleId="associacaoModeloRestricaoForm" >		
			<script>carregaMenu('mn_consultas_produtos_configurar_restricao');</script>
			<div class="secao_conteudo">
				<catalogo:contentBox title="Configurar Restrição">
					...
					<div class="fleft" style="width:36%; margin-bottom: 5px; margin-left: 5px;"> 
						<div class="label-form-bold " style="width: 110px;">
							<label for=idFabricanteSearch>Fabricante:<font size="1px" color="#EEB422">*</font></label>
						</div>
					...
					<div class="fleft" style="width: 31%; margin-bottom: 5px; margin-left: 5px;"> 
						<div class="label-form-bold " style="width: 60px;">
							<label for="idModeloSearch">Modelo:</label>
						</div>
					...
					<catalogo:contentBox title="Resultado da Pesquisa">	
						<display:table name="grupoProdutoResultList" id="grupoProdutoResultTO" pagesize= "15" cellspacing="0" cellpadding="0" style="width: 99%;" requestURI = "" class="tabela-padrao-new tablesorter table_body" >
							<display:column title="Tipo Produto" sortable="true" headerClass="sortable" style="width: 150px;">
								<html:link href="javascript:editar(${grupoProdutoResultTO.idGrupoProduto})">
									${grupoProdutoResultTO.tipoProdutoTO.nmTipoProduto}
								</html:link>
							</display:column>
							<display:column title="Fabricante" sortable="true" headerClass="sortable" style="width: 150px;">
								<html:link href="javascript:editar(${grupoProdutoResultTO.idGrupoProduto})">
									${grupoProdutoResultTO.fabricanteTO.nmFabricante}
								</html:link>
							</display:column>
							<display:column title="Modelo" sortable="true" headerClass="sortable" >
								<html:link href="javascript:editar(${grupoProdutoResultTO.idGrupoProduto})">
									${grupoProdutoResultTO.nmGrupoProduto}
								</html:link>
							</display:column>
									...
									<td colspan="5" style="text-align: left;">
									 	<table class="tabelaIcones" cellpadding="0" cellspacing="0" style="width: 280px;">
											<tr>
												...
												<td width="15px;">
													<img src="/catalogo/static_server/img/bullets/icon-available.png" alt="Com Restrição" /></td>
												<td width="150px;">Com Restri&ccedil;&atilde;o</td>
												<td width="15px;">
													<img src="/catalogo/static_server/img/bullets/icon-unavailable.png" alt="Sem Restrição" /></td>
				...
				<c:if test="${cadastrar}">
					<catalogo:contentBox title="Configurar Restrição" requiredFields="true">
						<div id="teste" ></div>
						<div class="fleft" style="width: 31%; margin-bottom: 5px; margin-left: 5px;"> 
						<div class="label-form-bold " style="width: 85px;">
							<label for="idModeloSearch">Modelo:<font size="1px" color="#EEB422">&nbsp;&nbsp;&nbsp;</font></label>
						</div>
						&nbsp;${nmModelo}
						</div>
						<div class="condicoes" style="width: 700px; float: left; margin-top: 5px; height: 40px;"> 
							<c:if test="${modeloTipoProdutoRestricaoTOList != null}">
								<c:forEach items="${modeloTipoProdutoRestricaoTOList}" var="tipoRestricaoTO">
									<p class="campoCondicao" style="float: left; display: inline;"> 
										<input type="hidden" id="idTipoCompativeisSelecionados" name="idTipoCompativeisSelecionados" value="" />
...
<%-- 										<select name=idTipoRestricaoArray id="idTipoRestricaoArray" style="width: 150px; height: 60px;" onchange="carregarTipoRestricao()" style="width: 160px;" >
												<option value="">-- Selecione --</option>
												<c:forEach items="${tipoProdutoList}" var="tipoRestricaoTOList" >
													<option value="${tipoRestricaoTOList.idTipoProduto}">${tipoRestricaoTOList.nmTipoProduto}</option>
												</c:forEach>
											</select> --%>
											&nbsp;&nbsp;&nbsp;
										</span>
										...
										<html:select property="idTipocompativelArray" styleId="idTipocompativelArray" style="width: 200px; height: 60px;" multiple="true">
											<c:forEach items="${tipoRestricaoTO.modeloTipoProdutoCompativelTOList}" var="tipoCompativelTO" >
												<html:option value="${tipoCompativelTO.tipoProdutoTO.idTipoProduto}">${tipoCompativelTO.tipoProdutoTO.nmTipoProduto}</html:option>
											</c:forEach>
										</html:select>
										
<%-- 									<select name="idTipocompativelArray" id="idTipocompativelArray" style="width: 200px; height: 60px;" multiple="true"  >
											<c:forEach items="${tipoRestricaoTO.modeloTipoProdutoCompativelTOList}" var="tipoCompativelTO" >
												<option value="${tipoCompativelTO.tipoProdutoTO.idTipoProduto}">${tipoCompativelTO.tipoProdutoTO.nmTipoProduto}</option>
											</c:forEach>
										</select> --%>
									</p>
								</c:forEach>
								<c:set var="contador" value="0" />
								<c:forEach items="${modeloTipoProdutoRestricaoTOList}" var="modeloTipoRestricaoTesteTO">
									<script>
				                    	var cont = ${contador};
				                    	
				                    	$jq('[id=idTipoRestricaoArray]').each(function(i){
				                    		if(i == cont) {
												$jq(this).val('${modeloTipoRestricaoTesteTO.tipoProdutoTO.idTipoProduto}');
				                    ...
				                    <c:forEach items="${idTipoCompativeisSelecionados}" var="idTipoCompativeisSelecionados" >
				                    	<c:if test="${contador == contadorSelecionados}">
				                    		<script>
				                    			...
				                    			var cont = ${contador}; 
				                    			if (document.associacaoModeloRestricaoForm('idTipocompativelArray').value == null) {
					                    			for (var m = 0; m < idTipoCompativeisSelecionadosArray.length; m++ ) {
					                    				for (var k = 0; k < document.associacaoModeloRestricaoForm('idTipocompativelArray')[cont].length; k++ ) {
							                    			if (document.associacaoModeloRestricaoForm('idTipocompativelArray')[cont][k].value == idTipoCompativeisSelecionadosArray[m]) {
							                    				document.associacaoModeloRestricaoForm('idTipocompativelArray')[cont][k].selected = 'true';
							                    			}
							                    		}
					                    			}
					                    			document.associacaoModeloRestricaoForm('idTipocompativelArray')[cont].disabled = "disabled";
				                    			} else {
				                    				for (var m = 0; m < idTipoCompativeisSelecionadosArray.length; m++ ) {
					                    				for (var k = 0; k < document.associacaoModeloRestricaoForm('idTipocompativelArray').length; k++ ) {
							                    			if (document.associacaoModeloRestricaoForm('idTipocompativelArray')[k].value == idTipoCompativeisSelecionadosArray[m]) {
							                    				document.associacaoModeloRestricaoForm('idTipocompativelArray')[k].selected = 'true';
							                    			}
							                    		}
					                    			}
					                    			document.associacaoModeloRestricaoForm('idTipocompativelArray').disabled = "disabled";
				                    			}
						...
						<div class="botao" id="botao">
							<html:button property="botao_limpar_form" styleId="botao_limpar_form" onClick="cancel()" styleClass="btNavegacao74" value="Cancelar" alt="Cancelar" title=""/>
						</div>
					</catalogo:contentBox>
				</c:if>
			</div>
