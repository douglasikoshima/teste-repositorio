...
extern bool proCExecutarRelatorio2Det( st_AtendimentoRel *dados
                                     , st_vlAtendimentoRel *status
                                     , XMLGen *saida
                                     , DOMNode *dnode );

cWFAtdRel2Det::cWFAtdRel2Det(DOMNode *dnode, XMLGen *xml_g)
{
    entrada = dnode;
    saida = xml_g;
...
bool cWFAtdRel2Det::Executar()
{
    return proCExecutarRelatorio2Det( &m_stDados
                                    , &m_vlDados
                                    , saida
                                    , entrada );
}

void cWFAtdRel2Det::carregaDados()
{
    char *p;
    ...
    if (p=tx.walkTree( entrada, "Responsavel", 0 ),p) 
    {
        sprintf(m_stDados.responsavel,"%.*s",sizeof(m_stDados.responsavel)-1,p);
        m_vlDados.responsavel = 1;
		XMLString::release(&p);
    }

    if (p=tx.walkTree( entrada, "nmLoginUsuarioAtual", 0 ),p) 
    {
        sprintf(m_stDados.nmLoginUsuarioAtual,"%.*s",sizeof(m_stDados.nmLoginUsuarioAtual)-1,p);
        m_vlDados.nmLoginUsuarioAtual = 1;
		XMLString::release(&p);
    }

    if (p=tx.walkTree( entrada, "nmUsuarioAtual", 0 ),p) 
    {
        sprintf(m_stDados.nmUsuarioAtual,"%.*s",sizeof(m_stDados.nmUsuarioAtual)-1,p);
        m_vlDados.nmUsuarioAtual = 1;
		XMLString::release(&p);
    }
