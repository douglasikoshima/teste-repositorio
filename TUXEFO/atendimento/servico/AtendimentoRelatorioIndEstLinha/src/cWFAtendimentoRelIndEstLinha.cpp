...
extern bool proCExecutarRelatorioIEL( st_AtendimentoRel *dados
                                    , st_vlAtendimentoRel *status
                                    , XMLGen *saida
                                    , DOMNode *dnode );

cWFAtdRelIEL::cWFAtdRelIEL(DOMNode *dnode, XMLGen *xml_g)
{
    entrada = dnode;
    saida = xml_g;
...
bool cWFAtdRelIEL::Executar()
{
    ...
    return proCExecutarRelatorioIEL( &m_stDados, &m_vlDados, saida, entrada );
}

void cWFAtdRelIEL::carregaDados()
{
    char *p;
