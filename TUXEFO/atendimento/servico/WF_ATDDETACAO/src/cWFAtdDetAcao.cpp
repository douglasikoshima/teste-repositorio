...
extern bool proCPermitirAcaoProcMC(int _idPessoaUsuario,long _idAtendimento,int _idAtividade);

cWFAtdDetAcao::cWFAtdDetAcao(DOMNode *dnode, XMLGen *xml_g)
{
    entrada = dnode;
    saida = xml_g;
...
bool cWFAtdDetAcao::executar()
{
    ULOG_START("cWFAtdDetAcao::executar()"); 
    ...
    ULOG_END("cWFAtdDetAcao::executar()"); 

    return true;
}

void cWFAtdDetAcao::ObterDetalheAtd()
{
    ULOG_START("cWFAtdDetAcao::ObterDetalheAtd()"); 

    cWFAtendimento cwfAtendimento;

    if ( cwfAtendimento.ObtemDetalheAtend(m_stDados.idAtendimento,&detalheAtendimento) == false )
    {
        throw new TuxBasicSvcException("00E0000","Falha na obtenção do detalhe do processo");
    }

    ULOG_END("cWFAtdDetAcao::ObterDetalheAtd()"); 
}

void cWFAtdDetAcao::ObterAcoes(XMLGen* _montagem)
{
    ULOG_START("cWFAtdDetAcao::ObterAcoes()"); 
    ...
    for ( int i = 0; i < nAcoes ; i++ )
    {
        ...
        ULOG("idFluxoFase    : %d", af->idFluxoFase);
        ULOG("idAtividade    : %d", af->idAtividade);
        ULOG("dsAtividade    : %s", af->dsAtividade.c_str());
        ULOG("nmFuncValidacao: %s", af->nmFuncValidacao.c_str());
           ...
           || (dados.inCri && strcmp(nmFuncValidacaoAt.c_str(),af->nmFuncValidacao.c_str())) )
        {
            ...
            if ( cwfCondicaoAparicao.ObtemWFCndAcoes(&dadosCondicao,&statusCondicao) )
            {
                ...
                // Processos Meu Cliente2 não podem ser encerrados/cancelados/fechados por analistas BKO/FO
                else if ( strcmp(detalheAtendimento.sgFluxoAtendimento,"MC2") == 0
                        && (af->idAtividade == CANCELAR_C ||
                            af->idAtividade == DEVOLVER_BKO_D ||
                            af->idAtividade == DEVOLVER_BKO_DCRI ||
                            af->idAtividade == DEVOLVER_BKO_DR ||
                            af->idAtividade == ENCERRAR_BKO_EB ||
                            af->idAtividade == ENCERRAR_BKO_EBR ||
                            af->idAtividade == ENCERRAR_BKO_EBS ||
                            af->idAtividade == ENCERRAR_BKO_MASSA_EBM ||
                            af->idAtividade == FECHAR_F ||
                            af->idAtividade == FECHAR_FR ||
                            //af->idAtividade == SUSPEITO_S ||
                            af->idAtividade == DEVOLVER_ANALISTA_MEU_CLIENTE)
                        )
                {
                    exibirAcao = 
                        proCPermitirAcaoProcMC(dados.idPessoaUsuario,m_stDados.idAtendimento,af->idAtividade);
                }
                else if ( strcmp(detalheAtendimento.sgFluxoAtendimento,"MC1") == 0 )
                {
                    if ( af->idAtividade == ADQUIRIR_PROCESSO_ADBKO ||
                         af->idAtividade == ADQUIRIR_PROCESSO_ADCRI ||
                         af->idAtividade == ADQUIRIR_PROCESSO_ADRC ||
                         //af->idAtividade == ENCAMINHAR_E ||
                         //af->idAtividade == SUSPEITO_S ||
                         af->idAtividade == ENCAMINHAR_CRI_ECRI
                       )
                    {
                        exibirAcao = false;
                    }
                ...
                if ( strcmp(detalheAtendimento.sgRegraEncaminhamento,"MC") == 0 && af->idAtividade == SUSPEITO_S )
                {
                    exibirAcao = false;
                }

                ULOG(" exibirAcao=%d", exibirAcao);
    ...
    ULOG_END("cWFAtdDetAcao::ObterAcoes()"); 
}

bool cWFAtdDetAcao::usuarioPossuiPerfilAbertura()
{
    ULOG_START("cWFAtdDetAcao::usuarioPossuiPerfilAbertura()"); 

    bool retorno = proCUsuarioPossuiPerfilAbertura(obterIdUsuario(),detalheAtendimento.idContato);

    ULOG_END("cWFAtdDetAcao::usuarioPossuiPerfilAbertura()"); 

    return retorno;
}

bool cWFAtdDetAcao::processoCriSimNao()
{
    ULOG_START("cWFAtdDetAcao::processoCriSimNao()"); 

    cWFAtdTratamentoCri cWFAtdTratamentoCri;

    bool retorno = cWFAtdTratamentoCri.getGrupoCri(m_stDados.idAtendimento);

    ULOG_END("cWFAtdDetAcao::processoCriSimNao()"); 

    return retorno;
}

void cWFAtdDetAcao::getUsuarioCri(int idPessoaUsuario)
{
    ULOG_START("cWFAtdDetAcao::getUsuarioCri()"); 
    ...
    if ( !cWFAtdTratamentoCri.getAtendimentoCri() )
    {
    ...
    ULOG_END("cWFAtdDetAcao::getUsuarioCri()"); 
}

void cWFAtdDetAcao::carregaDados()
{
    ULOG_START("cWFAtdDetAcao::carregaDados()"); 
    ...
    ULOG_END("cWFAtdDetAcao::carregaDados()"); 
}
