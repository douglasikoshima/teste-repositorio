...
extern bool proCExecutarRelatorioITP( st_AtendimentoRel *dados
                                    , st_vlAtendimentoRel *status
                                    , XMLGen *saida
                                    , DOMNode *dnode );

cWFAtdRelITP::cWFAtdRelITP(DOMNode *dnode, XMLGen *xml_g)
{
    entrada = dnode;
    saida = xml_g;
...
bool cWFAtdRelITP::Executar()
{
    ...
    return proCExecutarRelatorioITP( &m_stDados, &m_vlDados, saida, entrada );
}

void cWFAtdRelITP::carregaDados()
{
    char *p;
