...
extern bool proCExecutarRelatorio1( st_AtendimentoRel *dados
                                  , st_vlAtendimentoRel *status
                                  , XMLGen *saida
                                  , DOMNode *dnode );

cWFAtdRel1::cWFAtdRel1(DOMNode *dnode, XMLGen *xml_g)
{
    entrada = dnode;
    saida = xml_g;
...
bool cWFAtdRel1::Executar()
{
    return proCExecutarRelatorio1( &m_stDados
                                 , &m_vlDados
                                 , saida
                                 , entrada );
}

void cWFAtdRel1::carregaDados()
{
    char *p;
    ...
    if (p=tx.walkTree( entrada, "qtdLinhasBloco", 0 ),p) // num linhas no bloco
    {
        ...
        if ( m_stDados.qtdLinhasBloco > NUM_MAX_LINHAS  )
        {
            m_stDados.qtdLinhasBloco = NUM_MAX_LINHAS ;
        }
