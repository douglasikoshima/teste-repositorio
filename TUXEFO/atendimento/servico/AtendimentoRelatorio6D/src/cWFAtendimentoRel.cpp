...
extern bool proCExecutarRelatorio6D( st_AtendimentoRel *dados
                                   , st_vlAtendimentoRel *status
                                   , XMLGen *saida
                                   , DOMNode *dnode );

cWFAtdRel6D::cWFAtdRel6D(DOMNode *dnode, XMLGen *xml_g)
{
    entrada = dnode;
    saida = xml_g;
...
bool cWFAtdRel6D::Executar()
{
    return proCExecutarRelatorio6D( &m_stDados
                                  , &m_vlDados
                                  , saida
                                  , entrada );
}

void cWFAtdRel6D::carregaDados()
{
    char *p;
    ...
    ULOG("Total=%d",m_vlDados.bloco == 1 ? m_stDados.bloco: -1);
