...
#include <string>
#include "../include/cWF_POACNAOACOFPC.h"
using namespace std;
...
DECLARE_TUXEDO_SERVICE(POACNAOACOF);

void implPOACNAOACOF::Execute( DOMNode*dnode, XMLGen*xml_g )
{
    ULOG_START("implPOACNAOACOF::Execute()");

    string sCode = "04I0000";
    string mCode = "Execucao de POACNAOACOF Concluida";

    cWF_POACNAOACOF rc(dnode, xml_g, getUser());
    ...
    ULOG_END("implPOACNAOACOF::Execute()");
}
...
cWF_POACNAOACOF::cWF_POACNAOACOF(DOMNode* dnode, XMLGen* xml_g, char *user) 
{
...
// Caso de Uso: Não aceitar oferta de retenção
//------------------------------------------------------------------------------

void cWF_POACNAOACOF::Executar() 
{
    cWF_Acao::Executar();
    ...
    gravarPessoaPortabilidadeHist("POACNAOACOF");

    // ==> SM324--DPR--DEZ/2006--Cassio
    registrarAcaoDPR(User.ToInt(),idContato,"POACNAOACOF");
    // <== SM324--DPR--DEZ/2006--Cassio

    SetMessage("Oferta de retenção não aceita para o processo de portout","S");
