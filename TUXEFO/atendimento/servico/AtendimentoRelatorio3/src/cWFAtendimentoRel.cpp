...
extern bool proCExecutarRelatorio3( st_AtendimentoRel *dados
                                  , st_vlAtendimentoRel *status
                                  , XMLGen *saida
                                  , DOMNode *dnode );

cWFAtdRel3::cWFAtdRel3(DOMNode *dnode, XMLGen *xml_g)
{
    entrada = dnode;
    saida = xml_g;
...
bool cWFAtdRel3::Executar()
{
    return proCExecutarRelatorio3( &m_stDados
                                 , &m_vlDados
                                 , saida
                                 , entrada );
}

void cWFAtdRel3::carregaDados()
{
    char *p;
    ...
    if (p=tx.walkTree( entrada, "idCargo", 0 ),p) 
    {
        m_stDados.idCargo = atoi(p);
        m_vlDados.idCargo = 1;
        XMLString::release(&p);
    }
    ...
    if (p=tx.walkTree( entrada, "idStatus", 0 ),p) 
    {
        m_stDados.idStatusUsuario = atoi(p);
        m_vlDados.idStatusUsuario = 1;
		XMLString::release(&p);
    }
