...
DECLARE_TUXEDO_SERVICE(ATDDETFRARV);

void implATDDETFRARV::Execute(DOMNode*dnode,XMLGen*xml_g) 
{
    ULOG_START("implATDDETFRARV::Execute()");

    char *msgErro = 0;
    char *codErro = 0;
    char *usuario;

    cWFAtdDetFrArv ob(dnode,xml_g);
    ...
    ULOG_END("implATDDETFRARV::Execute()");
}
