...
extern bool proCExecutarRelatorioSGC( st_AtendimentoRel *dados
                                    , st_vlAtendimentoRel *status
                                    , XMLGen *saida
                                    , DOMNode *dnode );

cWFAtdRelSGC::cWFAtdRelSGC(DOMNode *dnode, XMLGen *xml_g)
{
    entrada = dnode;
    saida = xml_g;
...
bool cWFAtdRelSGC::Executar()
{
    ...
    return proCExecutarRelatorioSGC( &m_stDados, &m_vlDados, saida, entrada );
}

void cWFAtdRelSGC::carregaDados()
{
    char *p;
