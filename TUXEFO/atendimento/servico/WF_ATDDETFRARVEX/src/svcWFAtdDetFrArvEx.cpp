...
DECLARE_TUXEDO_SERVICE( ATDDETFRARVEX );

void implATDDETFRARVEX::Execute(DOMNode*dnode,XMLGen*xml_g) 
{
    ULOG_START("implATDDETFRARVEX::Execute()");

    char *msgErro = 0;
    char *codErro = 0;
    char *usuario;

    cWFAtdDetFrArvEx ob(dnode,xml_g);

    if (usuario=getUser(),usuario) 
    {
        ob.setarIdUsuario(atoi(usuario));

        ULOGE("idUsuario=%d",ob.obterIdUsuario());
    ...
    ULOG_END("implATDDETFRARVEX::Execute()");
}
