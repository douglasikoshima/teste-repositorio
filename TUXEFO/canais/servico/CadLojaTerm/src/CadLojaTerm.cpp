...
#include "../../negocio/Lojista/include/Lojista/Lojista.hpp"
...
DECLARE_TUXEDO_SERVICE(CADLOJATERM);
...
static void consultarTerminais(DOMNode* dnode, XMLGen* xml_g, CStatusCode &statusCode)
{
	...
	{	
		if(NO_DATA_FOUND == e.eCode){		
			...
			statusCode.setMsg("Não há terminais");
...
static void alterarTerminal(DOMNode* dnode, XMLGen* xml_g, CStatusCode &statusCode, char *user)
{
	...
	lojista.setIdPessoaDePara(iIdPessoaDePara);
...
static void incluirTerminal(DOMNode* dnode, XMLGen* xml_g, CStatusCode &statusCode, char *user)
{
	...
	if (intRC == NO_ERROR)
	{
		...
		iIdPessoaDePara = lojista.getIdPessoaDePara();
...
static void excluirTerminal(DOMNode* dnode, XMLGen* xml_g, CStatusCode &statusCode, char *user)
{
	...
	if (intRC == ERR_TERMINAL_CONFIGURADO_RECARGA )
	{
		...
		statusCode.setMsg("Este terminal não pode ser excluído, pois está configurado para realizar recarga e pagamento! Para excluir deverá alterar a configuração, removendo a opção de recarga e pagamento");
	}
...
static void listaDadosLojaTerminal(DOMNode* dnode, XMLGen* xml_g, CStatusCode &statusCode)
{
	...
	try
	{		
		lojista.listaDadosLojaTerminal(iIdTerminal, xml_g);
	...
	{	
		...
		{		
			...
			statusCode.setCod(STR_13W0005);				
			statusCode.setMsg("Não há dados na loja");
...
void implCADLOJATERM::Execute(DOMNode* dnode, XMLGen* xml_g) 
{
	CTuxHelperClever helper;
