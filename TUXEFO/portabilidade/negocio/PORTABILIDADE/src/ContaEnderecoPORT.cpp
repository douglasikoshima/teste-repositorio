...
char *CContaEndereco::getIdConta(void) {
    static char szAux[LEN_IDCONTA + LEN_EOS];

    strcpy(szAux, tContaEndereco.szIdConta);
    return szAux;
}

char *CContaEndereco::getIdTipoEnderecoCobranca(void) {
    static char szAux[LEN_IDTIPOENDERECOCOBRANCA + LEN_EOS];

    strcpy(szAux, tContaEndereco.szIdTipoEnderecoCobranca);
    return szAux;
}
