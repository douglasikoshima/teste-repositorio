...
DECLARE_TUXEDO_SERVICE(TUXATLYSURA);


void implTUXATLYSURA::Execute(DOMNode* dnode, XMLGen* xml_g)
{
	char*     pc_Linha    = NULL;
	...
	tuxfw_getlogger()->debug("Executando TuxAtlysURA");
	...
	*/
	...
	try
	{
		...
		else if (strcmp(pc_Operacao, opSetTipoFatura) == 0)
			po_PlugIn->setTipoFatura();

		else if (strcmp(pc_Operacao, opGetTipoFatura) == 0)
			po_PlugIn->getTipoFatura();
