...
DECLARE_TUXEDO_SERVICE(TuxProxyBE);


void implTuxProxyBE::Execute(DOMNode* dnode, XMLGen* xml_g)
{
	char*     pc_Linha    = NULL;
	char*     pc_Operacao = NULL;
	PlugInBE* po_PlugIn   = NULL;
	time_t    t_secStart;

	t_secStart = time(NULL);

	tuxfw_getlogger()->debug("Executando TuxProxyBE");
	...
	*/
	...
	try
	{
		...
		po_PlugIn = CreatorPlugInBE::getPlugIn(dnode, xml_g, pc_Linha);
		po_PlugIn->setUser(this->getUser());
		char*svcNm = po_PlugIn->getServiceName();
		if(svcNm != NULL && strlen(svcNm) == 0 )
