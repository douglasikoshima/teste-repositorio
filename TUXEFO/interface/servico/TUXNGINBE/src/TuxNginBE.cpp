...
DECLARE_TUXEDO_SERVICE(TUXNGINBE);


void implTUXNGINBE::Execute(DOMNode* dnode, XMLGen* xml_g)
{
	char*     pc_Linha    = NULL;
	char*     pc_Operacao = NULL;
	PlugInNGIN* po_PlugIn   = NULL;
	time_t    t_secStart;

	t_secStart = time(NULL);

	tuxfw_getlogger()->debug("Executando TuxNginBE");
	...
	*/
	...
	try
	{
		...
		po_PlugIn = CreatorPlugInBE::getPlugInNGIN(dnode, xml_g, pc_Linha);
		po_PlugIn->setUser(this->getUser());

		// Operacoes 'Get'


        if(strcmp(pc_Operacao, opGetAvailableServices) == 0)
            po_PlugIn->getAvailableServices();
        
        else if(strcmp(pc_Operacao, opGetDetalheAparelho) == 0)
            po_PlugIn->getDetalheAparelho();

		else if(strcmp(pc_Operacao, opGetDetalheLinha) == 0)
		...
		else if(strcmp(pc_Operacao, opGetVelocidade)== 0)
			po_PlugIn->getVelocidade();
