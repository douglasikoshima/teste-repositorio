...
/*
	...
	if(!bAchouCallCircle){
		tuxfw_getlogger()->debug("Nao Encontrado o PrimarySvc:svcNm=%s",cNmSrvATLYS); 
		throw new TuxBasicSvcException("24E0000", "Não Encontrado o PrimarySvc:svcNm requisitado.");
	}
*/
  ...
  /*
	...
	*/
	...
	while((auxDOMCallCircle = tuxhp->walkDOM(outputDOMNodeGetCallingCircle,"callCircleInfo", i)) != NULL ){
		...
		// Filtrar os Nros já expirados, para não exbir na Camada JAVA.
		// Se a Data de Expeirtacao(Atributo "exprDt") não for menor ou igual a Hoje
