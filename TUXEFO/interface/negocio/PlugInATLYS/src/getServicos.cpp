...
void PlugInATLYS::getAtributosTorpedo(int iNrLinha, char *cSvcNmTorpedo,  char *cSubject, char *cNickName)
{
	char cSbscrpId[256];
	...
	tuxfw_getlogger()->debug("PlugInATLYS::getAtributosTorpedo():callRemoteAPI()");
	...
	if( (outputDOMNodeGetSubscriptionInfo == NULL) || (strcmp(getLastStatusCode(),"24I0000") != 0) )
		return;
	...
	DOMNode *po_record;
	DOMNode *po_record_torp;
	int i = 0;
	int j = 0;


	//empresaNome é o nickName	
	//empresaMensagem ´subject

	while (!bServicoAtivo  && (po_record = tuxhp->walkDOM(outputDOMNodeGetSubscriptionInfo , "svcAsgmInfo", i++)) != NULL){

		if ((po_record = tuxhp->walkDOM(po_record, "primarySvc", 0)) != NULL){
			
			strcpy(cSvcNm, tuxhp->getAttrValue(po_record,"svcNm"));
			
			tuxfw_getlogger()->debug("cSvcNm = %s\r\n", cSvcNm);

			if (!strcmp (cSvcNm, cSvcNmTorpedo)){			
				bServicoAtivo = true;			
				break;
			}
		}
	...
	if (bServicoAtivo == true){
		
		tuxfw_getlogger()->debug("bServicoAtivo = true \r\n");
		
		po_record_torp = tuxhp->walkDOM(po_record, "svcAttr", 0);

		j = 0;
			
		while ((po_record = tuxhp->walkDOM(po_record_torp,"addlAttrs", j++)) != NULL){

			if (tuxhp->getAttrValue(po_record,"exprDtTm") == NULL){
				
				strcpy(cAttribute, tuxhp->getAttrValue(po_record,"attrNm"));
				
				if (!strcmp(cAttribute,"SUBJECT"))
					strcpy(cSubject, tuxhp->getAttrValue(po_record, "attrValue"));
				
				if (!strcmp(cAttribute,"NICKNAME"))					
					strcpy(cNickName, tuxhp->getAttrValue(po_record, "attrValue"));
