/*****************************************************************************
 *
 * Modulo:    FrdTipRemove
 * Arquivo:   FrdTipRemove.cpp
 * Proposito: servico do framework tuxedo que aciona servico pro*c
 * Historico:
 * Data        Autor                 Descricao
 * ----------  --------------------  -----------------------------------------
 * 17/06/2004  C_RECOliveira         Criacao
 * 17/06/2004  C_EDMartins           Criacao
 *
 ****************************************************************************/
/**************************************************************************
 * Notas:
 *
 **************************************************************************/

//Definicao Global
#define FrdTipRemoveCPP

//Header de Srv de Infra-Estrutura
#include<tuxfw.h>
#include "../../../negocio/admatdCmm/include/CSafePointer.h"
#include "../../../negocio/admatdCmm/include/CFrdTip.h"

//Macro de Definicao do Framework Tuxedo
DECLARE_TUXEDO_SERVICE(FrdTipRemove);

/**************************************************************************
 *  Funcao de Negocios:  FrdTipRemove
 *
 *  Descricao: chama funcao de negocios
 *
 *  Parametros de Entrada e Saida:
 *  DOMNode		*dnode    Nodos da Arvore DOM XML
 *  XMLGen		*xml_g    XML de entrada
 *
 *  Parametros de Saida:
 *  DOMNode		*dnode    Nodos da Arvore DOM XML
 *  XMLGen		*xml_g    XML de saida
 *
 *************************************************************************/
void implFrdTipRemove::Execute(DOMNode*dnode,XMLGen*xml_g)
{
   ULOG_START("implFrdTipRemove::Execute()");
	/* Chamada de Funcao de Negocios */
	CSafePointer oSafePointer;
	CTipoFeriado oTipoFeriado;
	char* cidTipoFeriado = oSafePointer.getTag( dnode, "idTipoFeriado", 0 );
	if( cidTipoFeriado == NULL )
	{
		setStatusCode("14E0000","idTipoFeriado esta nulo");
		ULOG_END("implFrdTipRemove::Execute()");
		return;
	}
	if( oTipoFeriado.Delete( cidTipoFeriado ) )
	{
		oTipoFeriado.ListAll();
		oTipoFeriado.GetXml("FeriadoNomeVO", xml_g);
		setStatusCode("14I0000","Operacao concluida com sucesso!");
	}
	else
		setStatusCode("14E0000","Falha na tentativa de DELETE");
		
	ULOG_END("implFrdTipRemove::Execute()");
}
