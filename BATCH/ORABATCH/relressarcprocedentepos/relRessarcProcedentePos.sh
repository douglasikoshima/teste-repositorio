...
ARQUIVO_EM_EXECUCAO=relRessarcProcedentePos.lock
...
source ${ORABATCH_DIR}/relRessarcProcedentePos_db_functions.sh
source ${ORABATCH_DIR}/relRessarcProcedentePos_files_functions.sh
...
then
	...
	do
		...
		then
			...
			# Conforme descrito documento funcional "RS04.2 Script para processamento de Ressarcimentos Procedentes do Pós Pago (RF03)"
			# No caso alternativo "A4 – Arquivo sem registros de 'A PEDIDO'"
			cargaArquivos ${ORABATCH_DIR_SQL}/RELRESSARCPROCEDENTEPOS.par ${arquivo}
			...
			executaQuery ${ORABATCH_CONNECTION} ${ORABATCH_DIR} sql/RELRESSARCPROCEDENTEPOS ${SEM_RETORNO} ${arquivo}
	...
	log ${LOG_PREFIX} "================================"
	log ${LOG_PREFIX} "\tVerifique se o processo esta em execucao: ps -ef | grep relRessarcProcedentePos"
