...
function executaQuery() {
	...
	if [ $V_LOG_RETURN != 0 ]
	then	
		retorno=`sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${ORABATCH_DIR_LOG}/${arquivo_sem_extensao}`
		sql_return_code=$?
	else
		sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${ORABATCH_DIR_LOG}/${arquivo_sem_extensao}
		sql_return_code=$?
	fi
