...
############################################################
export LOG_PREFIX="RELRESSARCIMENTO"
DIR_SHELL=`dirname $0`
cd ${DIR_SHELL} 
...
ARQUIVO_EM_EXECUCAO=relRessarcimento.lock

############################################################
# ForÁando Idioma do BD para utulizar acentuaÁ„o
############################################################
...
source ${ORABATCH_DIR}/relRessarcimento_db_functions.sh
source ${ORABATCH_DIR}/relRessarcimento_files_functions.sh
...
then
	...
	############################################################
	executaQuery ${ORABATCH_CONNECTION} ${ORABATCH_DIR} sql/${LOG_PREFIX}_APEDIDO ${SEM_RETORNO} ${ORABATCH_DIR_DATA_OUT}/${ARQUIVO_DATA_HORA}.txt
	...
	log ${LOG_PREFIX} "================================"
	log ${LOG_PREFIX} "\tVerifique se o processo esta em execucao: ps -ef | grep relRessarcimento"
