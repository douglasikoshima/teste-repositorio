...
ARQUIVO_EM_EXECUCAO=expurgoRessarcimento.lock
...
source ${ORABATCH_DIR}/expurgoRessarcimento_db_functions.sh
source ${ORABATCH_DIR}/expurgoRessarcimento_files_functions.sh
...
then
	...
	############################################################
	# Apaga os registros com mais de 90 dias
	############################################################
	log ${LOG_PREFIX} "Iniciando o processamento da tabela LOAD.PEDIDORESSARCIMENTO"
	executaQuery ${ORABATCH_CONNECTION} ${ORABATCH_DIR} sql/EXPURGORESSARCIMENTO ${COM_RETORNO}
	...
	log ${LOG_PREFIX} "================================"
	log ${LOG_PREFIX} "\tVerifique se o processo esta em execucao: ps -ef | grep expurgoRessarcimento"
