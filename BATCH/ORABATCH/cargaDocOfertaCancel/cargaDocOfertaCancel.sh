...
sql_plus() {
	...
	FILE_SPOOL="${DIR_LOG}/${BASE_NAME_DATA}${FILE_SQLPLUS_EXT}${FILE_LOG_EXT}"
	
	# Nome do arquivo de critica
	FILE_BAD="${DIR_BAD}/${BASE_NAME_DATA}${FILE_SQLPLUS_EXT}${FILE_BAD_EXT}"

	# Variaveis de retorno do plsql
	VO_CDERRO=
	VO_DSERRO=

	sqlplus "${SQL_CONN}" "@${SQL_PLUS_FILE}" "${ID_USUARIO_ALTERACAO}" "${FIELD_DELIMITER}" "${FILE_BAD}" > ${FILE_SPOOL} 2>&1
	RET_CODE=$?
	printm "Codigo retorno SQL*Plus: ${RET_CODE}"
	...
	then
		...
		then
			...
			# Verifica arquivo bad do sqlplus
			if test -s "${FILE_BAD}"
			then
				printm "SQL*Plus bad file (ARQUIVO DE CRITICA/NAO ENCONTRADO): ${FILE_BAD}"
			elif test -e "${FILE_BAD}"
			then
				rm -f "${FILE_BAD}"
			fi
		fi
