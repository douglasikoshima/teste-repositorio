...
############################################################
export LOG_PREFIX="cargaLoginClientesVivo2"
DIR_SHELL=`dirname $0`
cd ${DIR_SHELL} 
...
ARQUIVO_EM_EXECUCAO=cargaLoginClientesVivo2.lock
...
source ${ORABATCH_DIR}/cargaLoginClientesVivo2_db_functions.sh
source ${ORABATCH_DIR}/cargaLoginClientesVivo2_files_functions.sh
...
then
	...
	do
		...
		then
			...
			cargaArquivos ${ORABATCH_DIR_SQL}/cargaLoginClientesVivo2.par ${arquivo}
						
			#log ${LOG_PREFIX} "Iniciando o processamento da tabela LOAD.PEDIDORESSARCIMENTO"
					
			executaQuery ${ORABATCH_CONNECTION} ${ORABATCH_DIR} sql/cargaLoginClientesVivo2 ${SEM_RETORNO} ${arquivo}
