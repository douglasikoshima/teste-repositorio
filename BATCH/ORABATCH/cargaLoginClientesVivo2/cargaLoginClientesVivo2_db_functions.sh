...
function executaQuery() {
	...
	if [ $V_LOG_RETURN != 0 ]
	then			
		retorno=`sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${ORABATCH_DIR_BAD}/${arquivo_sem_extensao}.bad` 
		sql_return_code=$?
		log ${LOG_PREFIX} "\tRetorno de $V_SQL_FILE_NAME.sql: [$retorno]"
	else		
		sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${ORABATCH_DIR_BAD}/${arquivo_sem_extensao}.bad 
		sql_return_code=$?
	fi
	log ${LOG_PREFIX} "\tExecucao de ${V_SQL_FILE_NAME}.sql finalizada: [`date`]"
	
	#Se o arquivo bad for criado vazio, ele sera removido
	if [ `cat ${ORABATCH_DIR_BAD}/${arquivo_sem_extensao}.bad | wc -l` -eq 0 ]
	then      
      rm ${ORABATCH_DIR_BAD}/${arquivo_sem_extensao}.bad  
	fi
