...
#  Criado por.: Williams Santos 08/01/2016
...
DECLARE
    ...
    (   
       ...
       , DSSENHAVIVO2      ACESSO.LOGINMARCAS.DSSENHAVIVO2%TYPE
       , TPCLIENTE         ACESSO.LOGINMARCAS.TPCLIENTE%TYPE        
       , DSURLFACEBOOK     ACESSO.LOGINMARCAS.DSURLFACEBOOK%TYPE       
    );
    
    TYPE T_LOGINMARCAS IS TABLE OF TB_LOGINMARCAS;    
    V_LOGINMARCAS T_LOGINMARCAS;

    CURSOR CMP IS 
    SELECT CCG.N1 AS NRDOCUMENTO
         , CCG.N2 AS DSSENHAVIVO2
         , CCG.N3 AS TPCLIENTE
         , CCG.N4 AS DSURLFACEBOOK
      FROM LOAD.CARGACONTATOGRUPO CCG
	 WHERE LENGTH(CCG.N3) = 2;
    ...
    FETCH CMP BULK COLLECT INTO V_LOGINMARCAS LIMIT V_LIMITE;
        ...
        LOOP
           ...
           BEGIN
                ...
                IF (V_LOGINMARCAS(DOC).TPCLIENTE IS NULL OR 
                    V_LOGINMARCAS(DOC).NRDOCUMENTO IS NULL OR                    
                    V_LOGINMARCAS(DOC).DSSENHAVIVO2 IS NULL) 
                THEN
                
                    DBMS_OUTPUT.PUT_LINE(V_LOGINMARCAS(DOC).NRDOCUMENTO||';'||V_LOGINMARCAS(DOC).DSSENHAVIVO2||';'||V_LOGINMARCAS(DOC).TPCLIENTE||';'||V_LOGINMARCAS(DOC).DSURLFACEBOOK);
                ...
                ELSE
                    ...
                    THEN
                        ...
                        INSERT INTO ACESSO.LOGINMARCAS ALM (ALM.IDLOGIN
                         ...
                         VALUES (ACESSO.LOGINMARCASSQ.NEXTVAL --V_LOGINMARCAS(DOC).IDLOGIN
                               , NULL --V_LOGINMARCAS(DOC).DSEMAIL
                               ...
                               , 'VIVO2' --V_LOGINMARCAS(DOC).CANAL
                               , 0       --V_LOGINMARCAS(DOC).INSENHAUNIFICADA
                               , V_LOGINMARCAS(DOC).DSURLFACEBOOK
                               , NULL    --V_LOGINMARCAS(DOC).DSSENHATVANTAGENS
                               , V_LOGINMARCAS(DOC).DSSENHAVIVO2
                    ...
                    ELSE
                        ...
                        UPDATE ACESSO.LOGINMARCAS LM
                           SET DSSENHAVIVO2 = V_LOGINMARCAS(DOC).DSSENHAVIVO2
                             , INVALIDACAOPJ = 0
                             , INSENHAUNIFICADA = 0 
                             , DSURLFACEBOOK = V_LOGINMARCAS(DOC).DSURLFACEBOOK
