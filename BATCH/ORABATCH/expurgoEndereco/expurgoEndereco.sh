...
############################################################
export LOG_PREFIX="expurgoEndereco"
DIR_SHELL=`dirname $0`
cd ${DIR_SHELL} 
...
ARQUIVO_EM_EXECUCAO=expurgoEndereco.lock
...
source ${ORABATCH_DIR}/expurgoEndereco_db_functions.sh
source ${ORABATCH_DIR}/expurgoEndereco_files_functions.sh
...
then
	...
	processoEmExecucao "INICIAR"
			...
			executaQuery ${ORABATCH_CONNECTION} ${ORABATCH_DIR} sql/expurgoEndereco ${SEM_RETORNO} 
	...
	log ${LOG_PREFIX} "================================"
	log ${LOG_PREFIX} "	Verifique se o processo esta em execucao: ps -ef | grep expurgoEndereco"
