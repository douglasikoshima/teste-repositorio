...
function executaQuery() {
	...
	if [ $V_LOG_RETURN != 0 ]
	then	
		retorno=`sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${ORABATCH_DIR_DATA_IN}/${arquivo_sem_extensao}.txt`
		sql_return_code=$?
	else
  	
    # Nome do arquivo de spool do sqlplus
	  FILE_SQL_LOG="$ORABATCH_DIR_LOG/movimentacaoPupSdp_DataDeCorte_${LOG_HOJE}.log"
    
    SQL_PLUS_FILE_CONSULTA=${V_ORABATCH_DIR}/${V_SQL_FILE_NAME}_DataDeCorte.sql
    log ${LOG_PREFIX} "Nome do Arquivo ${SQL_PLUS_FILE_CONSULTA}"
    
    retorno=`sqlplus -s ${V_ORABATCH_CONNECTION} @${SQL_PLUS_FILE_CONSULTA} ${FILE_SQL_LOG}`		
		sql_return_code=$?
			
    VO_DATACORTE=$(cat "${FILE_SQL_LOG}" | egrep '^VO_DATACORTE' | sed 's/.*VO_DATACORTE *//')    
    log ${LOG_PREFIX} "VO_DATACORTE: ${VO_DATACORTE}"
    
    arquivoDATA=${ORABATCH_DIR_DATA_IN}/${arquivo_sem_extensao}_${VO_DATACORTE}.txt        		
		sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${arquivoDATA} ${VO_DATACORTE}
		sql_return_code=$?
	...
	#Se o arquivo bad for criado vazio, ele sera removido
    if [ `cat ${arquivoDATA} | wc -l` -eq 5 ]
	then        
      rm ${arquivoDATA}  
	fi	
	rm ${FILE_SQL_LOG}
