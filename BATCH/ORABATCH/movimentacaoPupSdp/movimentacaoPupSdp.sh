...
############################################################
export LOG_PREFIX="movimentacaoPupSdp"
DIR_SHELL=`dirname $0`
cd ${DIR_SHELL} 
...
ARQUIVO_EM_EXECUCAO=movimentacaoPupSdp.lock
NOME_ARQUIVO=movimentacaoPupSdp
...
source ${ORABATCH_DIR}/movimentacaoPupSdp_db_functions.sh
source ${ORABATCH_DIR}/movimentacaoPupSdp_files_functions.sh
...
then
	...
	processoEmExecucao "INICIAR"
			...
			executaQuery ${ORABATCH_CONNECTION} ${ORABATCH_DIR} sql/movimentacaoPupSdp ${SEM_RETORNO} ${NOME_ARQUIVO}
	...
	log ${LOG_PREFIX} "================================"
	log ${LOG_PREFIX} "	Verifique se o processo esta em execucao: ps -ef | grep movimentacaoPupSdp"
