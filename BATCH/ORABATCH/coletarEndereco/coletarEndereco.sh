...
############################################################
export LOG_PREFIX="coletarEndereco"
DIR_SHELL=`dirname $0`
cd ${DIR_SHELL} 
...
ARQUIVO_EM_EXECUCAO=coletarEndereco.lock
...
source ${ORABATCH_DIR}/coletarEndereco_db_functions.sh
source ${ORABATCH_DIR}/coletarEndereco_files_functions.sh
...
then
	...
	processoEmExecucao "INICIAR"
			...
			executaQuery ${ORABATCH_CONNECTION} ${ORABATCH_DIR} sql/coletarEndereco ${SEM_RETORNO} 
	...
	log ${LOG_PREFIX} "================================"
	log ${LOG_PREFIX} "	Verifique se o processo esta em execucao: ps -ef | grep coletarEndereco"
