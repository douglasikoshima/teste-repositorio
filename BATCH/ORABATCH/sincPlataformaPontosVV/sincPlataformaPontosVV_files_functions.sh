...
function apagaAquivos() {
	...
	#chavePesquisa=${2}	
	chavePesquisa='ParqueMovelVV_*.txt'
	...
	#Recupera arquivo maior que x dias, como "-mtime" eh em dias
	log ${LOG_PREFIX} "\tIniciando a remocao de arquivos em historico"
	for arquivo in $(find ${caminho}/${chavePesquisa} -mtime +${quantidadeDias})
	do
...
function move_arquivos_para_hist () {
	...
	for arquivo in ${ORABATCH_DIR_DATA_OUT}/${ARQUIVO_BASE}*.[tT][xX][tT]
	do
		# Verifica se o arquivo existe (as vezes o for..in.. nao acha o arquivo e retorna o valor de pesquisa)
		if [ -f ${arquivo} ]
		then
			arquivo_sem_extensao=`basename $arquivo .txt`
			log ${LOG_PREFIX} "	Verificando se o arquivo ${arquivo_sem_extensao} eh mais antigo que o arquivo ${ARQUIVO_DATA}"
			if [ "$arquivo_sem_extensao" \< "${ORABATCH_DIR_DATA_OUT}/$ARQUIVO_DATA" ] 
			then
				log ${LOG_PREFIX} "	Movendo o arquivo ${arquivo} para ${ORABATCH_DIR_HIST}"
				mv ${arquivo} ${ORABATCH_DIR_HIST}
