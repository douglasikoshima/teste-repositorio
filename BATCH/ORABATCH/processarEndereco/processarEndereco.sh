...
############################################################
export LOG_PREFIX="processarEndereco"
DIR_SHELL=`dirname $0`
cd ${DIR_SHELL} 
...
ARQUIVO_EM_EXECUCAO=processarEndereco.lock
ARQUIVO_BASE=BASESRE_
...
source ${ORABATCH_DIR}/processarEndereco_db_functions.sh
source ${ORABATCH_DIR}/processarEndereco_files_functions.sh
...
then
	...
	processoEmExecucao "INICIAR"
		...
		executaQuery ${ORABATCH_CONNECTION} ${ORABATCH_DIR} sql/processarEndereco ${SEM_RETORNO} ${ARQUIVO_DATA_HORA} ${arquivo_sem_extensao}
	...
	log ${LOG_PREFIX} "================================"
	log ${LOG_PREFIX} "	Verifique se o processo esta em execucao: ps -ef | grep processarEndereco"
