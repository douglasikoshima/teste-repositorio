...
function executaQuery() {
	...
	if [ $V_LOG_RETURN != 0 ]
	then	
		log ${LOG_PREFIX} "LN 41"
		retorno=`sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${ORABATCH_DIR_LOG}/${arquivo_sem_extensao}.txt`
		sql_return_code=$?
	else
		log ${LOG_PREFIX} "LN 45"
		sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${ORABATCH_DIR_DATA_IN}/${arquivo_sem_extensao}.txt
		sql_return_code=$?
	fi
	log ${LOG_PREFIX} "	Execucao de ${V_SQL_FILE_NAME}.sql finalizada: [`date`]"
	
	if [ `cat ${ORABATCH_DIR_DATA_IN}/${arquivo_sem_extensao}.txt | wc -l` -eq 0 ]
	then      
      rm ${ORABATCH_DIR_DATA_IN}/${arquivo_sem_extensao}.txt  
	fi
