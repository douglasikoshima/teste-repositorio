...
############################################################
export LOG_PREFIX="extracaoUsuario"
DIR_SHELL=`dirname $0`
cd ${DIR_SHELL} 
...
# O arquivo tem esta estrutura: LOGINS_ATIVOS_DDMMAAAAHHMISS.txt
ARQUIVO_BASE=LOGINS_ATIVOS_
#ARQUIVO_DATA=${ARQUIVO_BASE}`date +"%Y%m%d"`
ARQUIVO_DATA=${ARQUIVO_BASE}`date +"%d%m%Y"`
#ARQUIVO_DATA_HORA=${ARQUIVO_BASE}`date +"%Y%m%d%H%M%S"`
...
ARQUIVO_EM_EXECUCAO=extracaoUsuario.lock
...
source ${ORABATCH_DIR}/extracaoUsuario_db_functions.sh
source ${ORABATCH_DIR}/extracaoUsuario_files_functions.sh
...
then
	...
	############################################################
	executaQuery ${ORABATCH_CONNECTION} ${ORABATCH_DIR} sql/${LOG_PREFIX} ${COM_RETORNO} ${ORABATCH_DIR_DATA_OUT}/${ARQUIVO_DATA_HORA}.txt
	...
	log ${LOG_PREFIX} "================================"
	log ${LOG_PREFIX} "\tVerifique se o processo esta em execucao: ps -ef | grep extracaoUsuario"
