...
function executaQuery() {
	...
	if [ $V_LOG_RETURN != 0 ]
	then	
		retorno=`sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql` 
		
		sql_return_code=$?
	else
		sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql 
		#${ORABATCH_DIR_LOG}/${arquivo_sem_extensao}.log
		sql_return_code=$?
	fi
...
function cargaArquivos() {
	...
	log ${LOG_PREFIX} "\tarquivo_sem_extensao: ${arquivo_sem_extensao}"
    #log=${ORABATCH_DIR_LOG}/${arquivo_sem_extensao}.log
	${SQLLDR} userid=${ORABATCH_CONNECTION} parfile=${arquivoPar} data=${arquivoCarga} bad=${ORABATCH_DIR_BAD}/${arquivo_sem_extensao}.bad discard=${ORABATCH_DIR_BAD}/${arquivo_sem_extensao}.dsc
	RETCODE=$?
