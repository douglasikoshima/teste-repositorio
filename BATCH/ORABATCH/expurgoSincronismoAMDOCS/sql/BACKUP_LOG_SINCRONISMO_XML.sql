...
SPOOL &1;
	SELECT IDLOGSINCRONISMOXML
		   || '|' || IDLOGSINCRONISMO
           || '|' || REPLACE(REPLACE(dbms_lob.substr( XMLIN, 4000, 1 ),CHR(13),' '),CHR(10),' ') 
		   || '|' || REPLACE(REPLACE(dbms_lob.substr( XMLOUT, 4000, 1 ),CHR(13),' '),CHR(10),' ')
		   || '|' || DTLOG
	  FROM CONVERGENTE_OW.LOG_SINCRONISMO_XML
