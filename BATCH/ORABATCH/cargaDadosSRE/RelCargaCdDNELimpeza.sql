...
UNION ALL
SELECT tab.NOM_UF || ';' || tab.DSC_LOCALIDADE || ';' || tab.NUM_CEP || ';' || (SELECT COUNT (1) FROM LOGRADOURO LOGRAD WHERE LOGRAD.COD_LOCALIDADE = tab.COD_LOCALIDADE) count_loc 
FROM (SELECT UF.NOM_UF, LOC.COD_LOCALIDADE, LOC.DSC_LOCALIDADE, C.NUM_CEP 
            ...
            AND TRUNC (LOC.dat_carga) < (SELECT TRUNC (MAX (pc.dat_carga)) FROM processo_carga pc)) tab;
