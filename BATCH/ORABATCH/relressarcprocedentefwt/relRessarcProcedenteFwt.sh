...
#  Criado por.: Lucas Gomes Fenelon (24/08/2015)
...
############################################################
export LOG_PREFIX="PROCEDENTEFWT"
DIR_SHELL=`dirname $0`
cd ${DIR_SHELL} 
...
# O arquivo que vem do DW tem esta estrutura: MMDD
ARQUIVO_BASE=D.SGS08.A848
# Variaveis para controle de retorno do SQLPLUS
SEM_RETORNO=0
COM_RETORNO=1
# Arquivo para controle de execucao
ARQUIVO_EM_EXECUCAO=relRessarcProcedenteFwt.lock
...
source ${ORABATCH_DIR}/relRessarcProcedenteFwt_db_functions.sh
source ${ORABATCH_DIR}/relRessarcProcedenteFwt_files_functions.sh
...
then
	...
	# Le os arquivos para carga, enviados pelo DW: D.SGS08.A848MMDD ".ped" ?
	...
	do
		...
		then
			...
			# O CAMPO TIPO DO EQUIPAMENTO PRECISA TER A STRING 'PED', senao sera rejeitado para o arquivo BAD
			# Conforme descrito documento funcional "RS01.3 Script para processamento de Ressarcimentos Procedentes do FWT (RF01)"
			# No caso alternativo "A4 – Arquivo sem registros de 'A PEDIDO'"
			cargaArquivos ${ORABATCH_DIR_SQL}/RELRESSARCPROCEDENTEFWT.par ${arquivo}
			...
			log ${LOG_PREFIX} "Parametro de entrada query (parte do nome do arquivo): ${arquivo:12:4}"
			executaQuery ${ORABATCH_CONNECTION} ${ORABATCH_DIR} sql/RELRESSARCPROCEDENTEFWT ${SEM_RETORNO} ${arquivo} ${arquivo:(-8):4}
