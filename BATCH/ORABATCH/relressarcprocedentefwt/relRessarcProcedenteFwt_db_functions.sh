#!/bin/bash
############################################################
#  BATCH......: relRessarcImprocedenteFwt
...
function executaQuery() {
	...
	if [ $V_LOG_RETURN != 0 ]
	then
		retorno=`sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${ORABATCH_DIR_LOG}/${arquivo_sem_extensao}-NAO-ENCONTADO.log "${V_PARAM2}"`		
		sql_return_code=$?
		log ${LOG_PREFIX} "\tRetorno de $V_SQL_FILE_NAME.sql: [$retorno]"
	else
		sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${ORABATCH_DIR_LOG}/${arquivo_sem_extensao}-NAO-ENCONTADO.log "${V_PARAM2}"
		sql_return_code=$?
	fi
...
function cargaArquivos() {
	...
	*) 
	...
	if [ "${RETCODE}" == "0" -o "${RETCODE}" == "1" -o "${RETCODE}" == "2" -o "${RETCODE}" == "3" ]
	then
