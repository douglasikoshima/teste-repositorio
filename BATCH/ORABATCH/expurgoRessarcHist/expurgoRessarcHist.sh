...
############################################################
export LOG_PREFIX="EXPURGORESSARCHIST"
DIR_SHELL=`dirname $0`
cd ${DIR_SHELL} 
...
ARQUIVO_EM_EXECUCAO=expurgoRessarcHist.lock
...
source ${ORABATCH_DIR}/expurgoRessarcHist_files_functions.sh
...
then
	...
	############################################################
	# Apaga os arquivos que estao na pasta HIST por mais de 90 dias
	############################################################
	# Apaga os arquivos historiocos improcedentes
	log ${LOG_PREFIX} "== Apagando arquivos historiocos do Relatorio de Ressarcimento"
	
	apagaAquivos ${ORABATCH_DIR_REL_RESS} ${PERIODO_LIMPEZA}
	#apagaAquivos ${ORABATCH_DIR_REL_RESS} * ${PERIODO_LIMPEZA}
	...
	log ${LOG_PREFIX} "================================"
	log ${LOG_PREFIX} "\tVerifique se o processo esta em execucao: ps -ef | grep expurgoRessarcHist"
