...
int main( int argc, char* argv[] )
{
    ...
    try
    {
        ...
        log.logInformation("Vai tentar obter o semáforo....\n");

        int iRet = Process(argv[0],LOCK);

        if(iRet)
        {
            log.logInformation("Existe outra instância desse processo no ar. Processo não será iniciado.\n");
            return -1;
        }
        log.logInformation("Iniciando o processamento de ativação de aparelhos...\n");

        cativaraparelhopc.processaBatch();

        log.logInformation("Processamento executado.");
    ...
    {
        Process(argv[0],UNLOCK);
        log.logError("erro desconhecido");
        return -1;
    }

    // Encerra.
    log.logInformation("Vai liberar o semáforo...");
    
    Process(argv[0],UNLOCK);

    log.logInformation("PRONTO - Semáforo liberado e processamento finalizado.");
