...
int main(int argc, char *argv[])
{
	...
	log.logInformation("Iniciando processo de mailing");

	log.logInformation("lendo parametros do arquivo de configuração");
	// ler arquivo de configuração com parametros
	if(!lerParametros(&param,"mailing.cfg"))
	{
		log.logInformation("erro ao ler parametros");
		return 0;
	}
	if(strlen(param.pathMailing) == 0)
	{
		log.logInformation("erro ao ler parametro de PATH de arquivo para fazer");
		return 0;
	}
	// coloca o path no MailingServico
	servico.setPathArquivo(param.pathMailing);
...
int lerParametros(stParametros *param,char*arquivo)
{
	...
	while(file.getLine(linha) == 1)
	{
		...
		else if(!strcmp(atributo,"pathMailing"))
		{
			strcpy(param->pathMailing,valor);
		}
		memset(&linha,0,sizeof(linha));
	}
	file.fechar();
