...
typedef struct
{
	...
	char szPathLogErros[_MAX_PATH + 1];
	char ctrlOFICIAL[_MAX_PATH + 1];
	char ctrlDW[_MAX_PATH + 1];
	char scriptUPDATE[_MAX_PATH + 1];
...
int abrirArquivo(FILE **pFile,const char *pszPath, const char *pszModo = "w");
int gravarRegistro(FILE *pFile, const char *sRegistro);
