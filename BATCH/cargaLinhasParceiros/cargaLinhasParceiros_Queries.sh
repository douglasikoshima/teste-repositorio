...
function executaQuery() {
	...
	log ${LOG_PREFIX} "\tV_PARAM1 [$V_PARAM1]"
	log ${LOG_PREFIX} "\tV_PARAM2 [$V_PARAM2]"
	
	log ${LOG_PREFIX} "\tIniciando a execucao de $V_SQL_FILE_NAME.sql: [`date`]"
	if [ $V_LOG_RETURN != 0 ]
	then
		retorno=`sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${V_PARAM1} ${V_ORABATCH_DIR} ${V_PARAM2}`		
		sql_return_code=$?
		log ${LOG_PREFIX} "\tRetorno de $V_SQL_FILE_NAME.sql: [$retorno]"
	else
		sqlplus -s ${V_ORABATCH_CONNECTION} @${V_ORABATCH_DIR}/$V_SQL_FILE_NAME.sql ${V_PARAM1}
		sql_return_code=$?
	fi
