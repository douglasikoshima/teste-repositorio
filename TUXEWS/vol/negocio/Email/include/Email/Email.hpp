...
public:
	...
	void	incluirEmail(int iIdPessoa);
	void	alterarEmail();
	void	excluirEmail();
	bool	verificarEmail(int iIdPessoa);
	bool	verificarTipoEmail();
	int     getEmail(XMLGen*gen,char* linha,char* ddd, bool bObtemPos = false);	
...
private:
	...
	void	incluirEmailDB(int iIdPessoa);
	void	alterarEmailDB();
	void	excluirEmailDB();
	bool	verificarEmailDB(int iIdPessoa);
