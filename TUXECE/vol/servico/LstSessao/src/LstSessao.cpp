...
DECLARE_TUXEDO_SERVICE(LSTSESSAO);

void implLSTSESSAO::Execute(DOMNode* dnode, XMLGen* xml_g) {
	...
	tuxfw_getlogger()->debug("LSTSESSAO");

    if ( atoi(pInSenha) != 1 )
	{
		if ( NULL == pcArea || NULL == pcNroLinha || NULL == pcCdSenha) {
			//ERROR(NRO_TP_CORR_ID_NE);
			//TAG_INEXISTENTE(XML_IN_ID_PESSOA);
			throw new TuxBasicSvcException("20", "TAG_INEXISTENTE");
		}
		if ( '\0' == *pcArea || '\0' == *pcNroLinha || '\0' == *pcCdSenha ) {
	...
	xml_g->addItem( "idPessoa", oLinha.getPessoa()->getIdPessoa() );
