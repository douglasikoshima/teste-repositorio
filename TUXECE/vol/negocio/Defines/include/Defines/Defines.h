...
struct stProtocolo{
	char exibeProtocolo[2];
	char inProtocolo[2];
	char inRelacionamento[2];
...
#define REG_CONTATO(iResult, valTag) \
	...
	{ \
		...
		{ \
			...
			pcTagXmlIn = helper.walkTree(dnode,"observacao", 0); \			
			if((pcTagXmlIn != NULL) && (strlen(pcTagXmlIn) > 0)) \
				oLinhaReg.setObservacaoRegContato(pcTagXmlIn); \
...
#define REG_CONTATO_PROTOCOLO(iResult, valTag, protocolo) \
	int iResult = 0; \
	stProtocolo protocolo; \
	...
	{ \
		...
		char*  idSistemaOrigem = NULL; \
		char*  idTipoAberturaProtocolo = NULL; \
		char*  codigoFuncionalidade = NULL; \
		if(valTag) \
		{ \
			pcTagXmlIn = helper.walkTree(dnode,"inRegistrarContato", 0); \
			ASSERT_INT(iInRegistrarContato, pcTagXmlIn, "inRegistrarContato"); \
		...
		{ \
			...
			pcTagXmlIn = helper.walkTree(dnode,"cdContato", 0); \
			ASSERT_STR(pcTagXmlIn, "cdContato"); \
			nrProtocolo = helper.walkTree(dnode,"nrProtocolo", 0); \
			idSistemaOrigem = helper.walkTree(dnode,"idSistemaOrigem", 0); \
			idTipoAberturaProtocolo = helper.walkTree(dnode,"idTipoAberturaProtocolo", 0); \
			codigoFuncionalidade = helper.walkTree(dnode,"cdContato", 0); \
			pcTagXmlIn = helper.walkTree(dnode,"idCanal", 0); \
			ASSERT_INT(iIdCanal, pcTagXmlIn, "idCanal"); \
			pcTagXmlIn = helper.walkTree(dnode,"idTerminal", 0); \
			if((pcTagXmlIn != NULL) && (strlen(pcTagXmlIn) > 0)) \
			...
			oLinhaReg.getPessoa()->setIdTipoRelacionamento(iIdTipoRelacionamento); \
			oLinhaReg.getPessoa()->getRel()->setIdCanal(iIdCanal); \		
			oLinhaReg.registrarContato(iIdTerminal, this->getUser(),nrProtocolo, idSistemaOrigem,idTipoAberturaProtocolo,codigoFuncionalidade); \
			strcpy(protocolo.nrProtocolo,oLinhaReg.getNrProtocolo()); \
		} \
		iResult = 0; \
	} \
	catch(...){ iResult = 1; }
