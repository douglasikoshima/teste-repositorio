...
public:
	...
	void montaArvorePessoaCanal(long idPessoa, int idCanal, XMLGen* xml);

private:

	void consultaArvore(int idCanal, list<TP_ARVORE_PROFILE_SQL> &lstProfile);

	void consultaArvorePessoaCanalValPosDB(long idPessoa,
										   int idCanal,
										   list <TP_ARVORE_PROFILE_SQL> &lstProfile);

	void consultaArvorePessoaCanalValLivreDB(long idPessoa,
											 int idCanal, 
											 list <TP_ARVORE_PROFILE_SQL> &lstProfile);

	void montaArvorePessoaCanalDB(long idPessoa, int idCanal, XMLGen* xml);
};
